---
title: "Trait why-i-need-stacks"
draft: true
---
```

;; title: why i need stacks NFT Contract
;; version: 1.0
;; summary: A non-fungible token contract for why i need stacks NFTs
...
;; token definitions
(define-non-fungible-token whyineedstacks uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://bafybeib2cctnbes2r2froy4g6a6yzlwl2zmh4xqmacqlooad7mkhzjwhmy/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? whyineedstacks next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? whyineedstacks token-id) false)))

(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? whyineedstacks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? whyineedstacks id) false)))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? whyineedstacks token-id tx-sender)))
...
(define-public (buy-in-ustx (id uint))
  (let (
      (owner (unwrap! (nft-get-owner? whyineedstacks id) (err ERR-NOT-FOUND)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? whyineedstacks token-id)))
