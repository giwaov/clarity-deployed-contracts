---
title: "Trait dual-stacking-v2-testing"
draft: true
---
```
...
(define-data-var bitcoin-blocks-per-year uint u6250)
...
(if is-in-mainnet
  (begin
    (var-set current-cycle-bitcoin-block-height u927055)
    (var-set snapshots-per-cycle u5)
    (var-set blocks-per-snapshot u40)
    (var-set buffer-blocks-per-cycle u50)
  )
  (begin
    (var-set current-cycle-bitcoin-block-height u155)
    (var-set snapshots-per-cycle u4)
    (var-set blocks-per-snapshot u3)
    (var-set buffer-blocks-per-cycle u12)
  )
)
...
(define-private (capture-participant-snapshot
  ...
  (let (
      ...
      (is-participant-enrolled-this-cycle (is-enrolled-this-cycle enrolled-address))
...
(define-read-only (meets-enrollment-minimum (user {
  ...
  (>=
    (+ (get-wallet-sbtc-balance (get address user))
      (get total (get-defi-sbtc-balance user))
    )
    (var-get min-sbtc-hold-required-for-enrollment)
  )
)
...
(define-read-only (get-defi-sbtc-balance (user {
  ...
  (contract-call? .sbtc-holdings-defi get-user-individual-sBTC-in-DeFIs
    user
  )
)
