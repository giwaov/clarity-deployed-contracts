---
title: "Trait staging-vault-usdc-v0"
draft: true
---
```
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;; TRAITS
;; ============================================================================
(impl-trait .staging-vault-traits-v0.reserve)
(impl-trait .staging-vault-traits-v0.tokenized-vault)
(impl-trait .staging-vault-traits-v0.lending)
(impl-trait .staging-vault-traits-v0.flashloan)
(use-trait flash-callback .staging-vault-traits-v0.flash-callback)
...
;; -- Core configuration
;; Mainnet: Bridged USDC (aeUSDC) on Stacks
(define-constant UNDERLYING 'SP3Y2ZSH8P7D50B0VBTSX11S7XSG24M1VB9YFQA4K.token-aeusdc)
...
(define-private (receive-underlying (amount uint) (account principal))
  (begin
    (try! (contract-call? 'SP3Y2ZSH8P7D50B0VBTSX11S7XSG24M1VB9YFQA4K.token-aeusdc transfer amount account current-contract none))
    (ok true)))

(define-private (send-underlying (amount uint) (account principal))
  (begin
    (try! (contract-call? 'SP3Y2ZSH8P7D50B0VBTSX11S7XSG24M1VB9YFQA4K.token-aeusdc transfer amount current-contract account none))
    (ok true)))

(define-private (ubalance)
  (unwrap-panic (contract-call? 'SP3Y2ZSH8P7D50B0VBTSX11S7XSG24M1VB9YFQA4K.token-aeusdc get-balance current-contract)))
...
(define-private (principal-ratio-reduction (amount uint))
  (calc-principal-ratio-reduction amount (var-get principal-scaled) (total-debt)))
      ...
      (if (get accrue states)
          ...
          (let ((next (next-index))
            ...
            (if (not (is-eq lidx nliq))
                ...
                false)
            (if (> treasury-lp u0)
                (try! (ft-mint? zft treasury-lp .staging-dao-treasury-v0))
                false)
  ...
  (let (
    ...
    (print {
      ...
      data: {
        ...
        total-borrowed: (+ (var-get total-borrowed) amount),
        index: idx
      }
    })
  ...
  (let (
    ...
    (print {
      ...
      data: {
        ...
        assets: (+ (var-get assets) interest-paid),
        index: idx
      }
    })
...
(define-public (flashloan
  ...
  (let ((states (var-get pause-states))
    ...
    (if (> fee u0)
      (try! (send-underlying fee .staging-dao-treasury-v0))
      false)
