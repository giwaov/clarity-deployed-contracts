---
title: "Trait dao-core-v3"
draft: true
---
```
;; Simple DAO core v3 (testing-only): STX-weighted voting and direct transfers.
    ...
    (if (get executed proposal)
      ...
      (if (< stacks-block-height (get end-height proposal))
        ...
        (if (proposal-passes proposal)
          (match (stx-transfer? (get amount proposal) tx-sender (get recipient proposal))
            ok-result (begin
              (map-set proposals { id: proposal-id } {
                proposer: (get proposer proposal),
                recipient: (get recipient proposal),
