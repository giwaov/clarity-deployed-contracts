---
title: "Trait test-controller-hbtc-v6"
draft: true
---
```
;; @contract Controller
...
(define-constant rf .test-reserve-fund-hbtc-v6)
...
(define-public (log-reward (reward uint) (is-positive bool))
  (let (
    (state (contract-call? .test-state-hbtc-v6 get-reward-state))
    (fees (get fees state))
    ...
    (try! (contract-call? .test-state-hbtc-v6 check-is-reward-enabled))
    (try! (contract-call? .test-hq-vaults-v6 check-is-rewarder contract-caller))
    (try! (contract-call? .test-state-hbtc-v6 check-max-reward reward))
...
(define-public (fund-transfers)
  (let (
    (pending (contract-call? .test-state-hbtc-v6 get-pending))
    ...
    (if (> pending-fees u0) (try! (contract-call? .test-reserve-hbtc-v6 transfer sbtc-token pending-fees fee-collector)) true)
    (if (> pending-rf u0) (try! (contract-call? .test-reserve-hbtc-v6 transfer sbtc-token pending-rf rf)) true)
...
(define-private (handle-profit 
  ...
  (let (
    ...
    (ok (try! (contract-call? .test-state-hbtc-v6 update-state 
      (list
...
(define-private (handle-loss-covered 
  ...
  (let (
    ...
    (if (> transfer-amount u0)
      (try! (contract-call? .test-reserve-fund-hbtc-v6 transfer sbtc-token transfer-amount reserve none))
      true
    )
