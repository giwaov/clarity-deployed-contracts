---
title: "Trait staging-proposal-init-vaults-mainnet-v0"
draft: true
---
```
...
(define-public (execute)
  (begin
    ...
    ;; Mints minimum liquidity to prevent share manipulation attacks
    ;; Uses as-contract to ensure proposal contract is the caller when vaults
    ;; call deposit (account = contract-caller), allowing access to DAO assets
    ;; via with-all-assets-unsafe
    ;; =========================================================================
    
    (try! (as-contract? ((with-all-assets-unsafe))
      (try! (contract-call? .staging-vault-stx-v0 initialize))
      (try! (contract-call? .staging-vault-sbtc-v0 initialize))
      (try! (contract-call? .staging-vault-ststx-v0 initialize))
      (try! (contract-call? .staging-vault-usdc-v0 initialize))
      (try! (contract-call? .staging-vault-usdh-v0 initialize))
      true))
    
    ;; =========================================================================
    ;; STEP 3: AUTHORIZE MARKET CONTRACT
