---
title: "Trait undo-faktory-pool"
draft: true
---
```

        ;; Title: sBTC-UNDO lp-token
        ;; Version: 1.0.0
        ...
        (define-constant CONTRACT_INITIALIZER 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.undo-faktory-dex)
        (define-constant SAINT 'SP000000000000000000002Q6VF78)
    
        ;; Define LP token
        (define-fungible-token sBTC-UNDO)
        ...
        (define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
            (begin
                ...
                (try! (ft-transfer? sBTC-UNDO amount sender recipient))
        ...
        (define-read-only (get-balance (who principal))
            (ok (ft-get-balance sBTC-UNDO who)))
    
        (define-read-only (get-total-supply)
            (ok (ft-get-supply sBTC-UNDO)))
        ...
        (define-public (swap-a-to-b (amount uint) (min-y-out uint))
            (let (
                ...
                (try! (as-contract (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.undo-faktory transfer dy-d CONTRACT sender none)))
                (print {
                    type: "buy",
                    sender: sender,
                    token-in: 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token,
                    amount-in: amount,
                    token-out: 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.undo-faktory,
                    amount-out: dy-d,
        ...
        (define-public (swap-b-to-a (amount uint) (min-y-out uint))
            (let (
                ...
                (try! (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.undo-faktory transfer amount sender CONTRACT none))
                ...
                (print {
                    type: "sell",
                    sender: sender,
                    token-in: 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.undo-faktory,
                    amount-in: amount,
        ...
        (define-public (add-liquidity (amount uint))
            (let (
                ...
                (try! (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.undo-faktory transfer dy-d sender CONTRACT none))
                (try! (ft-mint? sBTC-UNDO dk-d sender))
                (print {
                    type: "add-liquidity",
                    sender: sender,
                    token-a: 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token,
                    token-a-amount: dx-d,
                    token-b: 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.undo-faktory,
                    token-b-amount: dy-d,
                    lp-tokens: dk-d,
                    pool-reserves: (get-reserves-quote),
                    pool-contract: CONTRACT
        ...
        (define-private (add-liquidity-and-burn (amount uint))
            (let (
                ...
                (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token transfer dx-d sender CONTRACT none))
                (try! (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.undo-faktory transfer dy-d sender CONTRACT none))
                (try! (ft-mint? sBTC-UNDO dk-d SAINT))
                (print {
        ...
        (define-public (remove-liquidity (amount uint))
            (let (
                ...
                (dy-d (get dy delta))
                (dk-d (get dk delta)))
                (try! (ft-burn? sBTC-UNDO dk-d sender))
        ...
        (define-private (get-reserves)
            { 
              ...
              b: (unwrap-panic (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.undo-faktory get-balance CONTRACT))
            })
        ...
        (define-read-only (get-reserves-quote)
            (let (
                (reserves (get-reserves))
                (supply (ft-get-supply sBTC-UNDO)))
                {
                  dx: (get a reserves),
                  dy: (get b reserves),
                  dk: supply
        ...
        (define-public (initialize-pool (lowest uint) (highest uint)) ;; highest = origin minus lowest
          (begin
             ...
             (try! (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.undo-faktory transfer highest contract-caller CONTRACT none))
