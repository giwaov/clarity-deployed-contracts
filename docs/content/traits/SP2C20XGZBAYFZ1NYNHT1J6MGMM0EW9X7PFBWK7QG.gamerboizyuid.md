---
title: "Trait gamerboizyuid"
draft: true
---
```

;; title: gamerboizyuid NFT Contract
;; version: 1.0
;; summary: A non-fungible token contract for gamerboizyuid NFTs
...
;; token definitions
(define-non-fungible-token gamerboizyuid uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://bafybeifeop6vjdsj7onam73tllja5izvi7ipqxbrhx3t7u7mr2ihkugjhu/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? gamerboizyuid next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? gamerboizyuid token-id) false)))

(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? gamerboizyuid id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? gamerboizyuid id) false)))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? gamerboizyuid token-id tx-sender)))
...
(define-public (buy-in-ustx (id uint))
  (let (
      (owner (unwrap! (nft-get-owner? gamerboizyuid id) (err ERR-NOT-FOUND)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? gamerboizyuid token-id)))
