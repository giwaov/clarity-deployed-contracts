...
;; get-all-assets
;; ---------------------------------------------------------------------------
;; Returns all registered assets with their status
(define-read-only (get-all-assets)
  (let ((nonce (unwrap-panic (contract-call? .staging-assets-v0 get-nonce))))
    (ok {
      count: nonce,
      asset-0: (if (> nonce u0) (some (contract-call? .staging-assets-v0 get-status u0)) none),
      asset-1: (if (> nonce u1) (some (contract-call? .staging-assets-v0 get-status u1)) none),
      asset-2: (if (> nonce u2) (some (contract-call? .staging-assets-v0 get-status u2)) none),
      asset-3: (if (> nonce u3) (some (contract-call? .staging-assets-v0 get-status u3)) none),
      asset-4: (if (> nonce u4) (some (contract-call? .staging-assets-v0 get-status u4)) none),
      asset-5: (if (> nonce u5) (some (contract-call? .staging-assets-v0 get-status u5)) none),
      asset-6: (if (> nonce u6) (some (contract-call? .staging-assets-v0 get-status u6)) none),
      asset-7: (if (> nonce u7) (some (contract-call? .staging-assets-v0 get-status u7)) none),
      asset-8: (if (> nonce u8) (some (contract-call? .staging-assets-v0 get-status u8)) none),
      asset-9: (if (> nonce u9) (some (contract-call? .staging-assets-v0 get-status u9)) none)
    })))
...
;; - Vault LP token balances (zTokens in wallet)
;; - Market-vault collateral (any collateral-enabled tokens deposited)
(define-read-only (get-supplies-user (account principal))
  (let ((enabled-mask (contract-call? .staging-assets-v0 get-bitmap))
        ;; Get vault LP token balances (zTokens held in wallet)
        (vault-stx-balance (unwrap-panic (contract-call? .staging-vault-stx-v0 get-balance account)))
        (vault-sbtc-balance (unwrap-panic (contract-call? .staging-vault-sbtc-v0 get-balance account)))
        (vault-ststx-balance (unwrap-panic (contract-call? .staging-vault-ststx-v0 get-balance account)))
        (vault-usdc-balance (unwrap-panic (contract-call? .staging-vault-usdc-v0 get-balance account)))
        (vault-usdh-balance (unwrap-panic (contract-call? .staging-vault-usdh-v0 get-balance account))))
    ;; Try to get market-vault position (may not exist)
    (match (contract-call? .staging-market-vault-v0 get-position account enabled-mask)
      position
        (ok {
          account: account,
          ;; Vault LP tokens (not deposited as collateral)
          vault-balances: {
            stx: vault-stx-balance,
            sbtc: vault-sbtc-balance,
            ststx: vault-ststx-balance,
            usdc: vault-usdc-balance,
            usdh: vault-usdh-balance
          },
          ;; Market-vault collateral (any collateral-enabled tokens)
          market-collateral: (get collateral position)
        })
      err-code
        ;; No market-vault position exists - return vault balances only
        (ok {
          account: account,
          vault-balances: {
            stx: vault-stx-balance,
