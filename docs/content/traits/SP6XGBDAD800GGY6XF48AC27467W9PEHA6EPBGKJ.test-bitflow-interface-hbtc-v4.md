---
title: "Trait test-bitflow-interface-hbtc-v4"
draft: true
---
```
...
(use-trait stableswap-core .test-bitflow-traits-v2.stableswap-core-trait)
(use-trait xyk-core .test-bitflow-traits-v2.xyk-core-trait)
(use-trait swap-helper .test-bitflow-traits-v2.swap-helper-trait)
...
(define-constant price-base u100000000)                         ;; 10^8 = 100000000 (price base)
...
(define-public (bitflow-stable-swap-x-for-y
  ...
  (begin
    ...
    (try! (contract-call? .test-state-hbtc-v4 check-trading-auth (contract-of core-trait) (some (contract-of pool-trait)) (some (contract-of x-token-trait)) (some (contract-of y-token-trait))))

    (try! (contract-call? .test-reserve-hbtc-v4 transfer x-token-trait x-amount this-contract))
    (let (
...
(define-public (bitflow-stable-swap-y-for-x
  ...
  (begin
    ...
    (try! (check-is-swap-active))
    (try! (contract-call? .test-state-hbtc-v4 check-trading-auth (contract-of core-trait) (some (contract-of pool-trait)) (some (contract-of x-token-trait)) (some (contract-of y-token-trait))))
    (try! (contract-call? .test-reserve-hbtc-v4 transfer y-token-trait y-amount this-contract))
    (let (
...
(define-private (calculate-min-output-stableswap
  ...
  (min-output-arg uint))
  (let (
    (input-token-data (contract-call? .test-state-hbtc-v4 get-asset (contract-of input-token-trait)))
    (max-slippage (get max-slippage input-token-data))
    (output-token-base (get token-base (contract-call? .test-state-hbtc-v4 get-asset (contract-of output-token-trait))))
...
(define-private (check-is-price-stale (publish-time uint))
  (let (
    (staleness-window (contract-call? .test-state-hbtc-v4 get-staleness-window))
    (current-ts (unwrap-panic (get-stacks-block-info? time (- stacks-block-height u1))))
    (min-timestamp (- current-ts staleness-window))
  )
...
(define-private (calculate-min-output-xyk
  ...
  (let (
    (input-token-data (contract-call? .test-state-hbtc-v4 get-asset (contract-of input-token-trait)))
    (output-token-data (contract-call? .test-state-hbtc-v4 get-asset (contract-of output-token-trait)))
    (max-slippage (get max-slippage input-token-data))
