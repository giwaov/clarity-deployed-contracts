---
title: "Trait cyberbully"
draft: true
---
```
;; title: Cyberbully - Soulbound Token
;; version: 1.0.0
...
;; token definition
(define-non-fungible-token cyberbully uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://bafkreicg4wxivmobjrts527ujcp7m6khb2p6b2u7qhcgrq342kkyglgyey")
...
(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? cyberbully id))
)
...
(define-public (mint)
  (let ((token-id (var-get next-id)) (caller tx-sender))
    ...
    (try! (nft-mint? cyberbully token-id caller))
...
(define-public (mint-to (recipient principal))
  (let ((token-id (var-get next-id)) (caller (get-standard-caller)))
    ...
    (try! (nft-mint? cyberbully token-id recipient))
...
(define-private (airdrop-mint (recipient principal) (id uint))
  (if (is-some (map-get? has-minted recipient))
    id
    (match (nft-mint? cyberbully id recipient)
...
(define-public (burn (id uint))
  (let ((owner (unwrap! (nft-get-owner? cyberbully id) ERR-TOKEN-NOT-FOUND)))
    (asserts! (is-eq tx-sender owner) ERR-NOT-AUTHORIZED)
    (try! (nft-burn? cyberbully id owner))
    (ok true)
  )
)
