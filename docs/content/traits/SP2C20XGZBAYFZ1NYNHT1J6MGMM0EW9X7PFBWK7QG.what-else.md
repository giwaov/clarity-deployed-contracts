---
title: "Trait what-else"
draft: true
---
```

;; title: what else NFT Contract
;; version: 1.0
;; summary: A non-fungible token contract for what else NFTs
...
;; token definitions
(define-non-fungible-token whatelse uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://bafybeihvvttvmd4ow2x6phaso5tv6sth75n32zgviksbxj7ab5ahjjwzmu/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? whatelse next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? whatelse token-id) false)))

(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? whatelse id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? whatelse id) false)))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? whatelse token-id tx-sender)))
...
(define-public (buy-in-ustx (id uint))
  (let (
      (owner (unwrap! (nft-get-owner? whatelse id) (err ERR-NOT-FOUND)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? whatelse token-id)))
