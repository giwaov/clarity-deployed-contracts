---
title: "Trait st-ststxbtc-vault-init-1"
draft: true
---
```
...
(define-public (execute)
  (begin
    ;; Asset ID 11: zstSTXbtc (vault shares, not underlying token)
    (try! (contract-call? .st-assets insert .st-vault-ststxbtc
      { type: TYPE-PYTH, ident: STX-FEED-ID, callcode: (some CALLCODE-ZSTSTXBTC), max-staleness: MAX-STALENESS }))

    ;; Enable zstSTXbtc for collateral only (already done by insert, but explicit)
    (try! (contract-call? .st-assets enable .st-vault-ststxbtc true))

    ;; Disable underlying stSTXbtc token as collateral
