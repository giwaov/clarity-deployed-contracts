---
title: "Trait buried-in-plain-site"
draft: true
---
```
;; buried-in-plain-site
;; contractType: editions

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token buried-in-plain-site uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmawJ5UDwfgyT7FpMnkSxjecoHV6fMBYjzG1TLC5kRgVpL/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? buried-in-plain-site next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? buried-in-plain-site token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? buried-in-plain-site token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? buried-in-plain-site token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? buried-in-plain-site id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? buried-in-plain-site id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? buried-in-plain-site id) (err ERR-NOT-FOUND)))
