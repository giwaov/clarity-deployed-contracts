...
;; TRAITS
;; ============================================================================
(use-trait ft-trait .sm-ft-trait.ft-trait)
(impl-trait .sm-market-trait.market-trait)
...
;; NOTE: These IDs depend on asset registration order in proposal-init-assets.clar
...
;; -- Contract references
(define-constant ZEST-STX-WRAPPER-CONTRACT .sm-wstx)
...
(define-private (check-dao-auth)
  (ok (asserts! (is-eq tx-sender .sm-dao-executor) ERR-AUTH)))

;; -- Vault routing ----------------------------------------------------------

(define-private (vault-accrue (aid uint))
  (if (is-eq aid STX) (contract-call? .sm-vault-stx accrue)
  (if (is-eq aid sBTC) (contract-call? .sm-vault-sbtc accrue)
  (if (is-eq aid stSTX) (contract-call? .sm-vault-ststx accrue)
  (if (is-eq aid USDC) (contract-call? .sm-vault-usdc accrue)
  (if (is-eq aid USDH) (contract-call? .sm-vault-usdh accrue)
  (if (is-eq aid stSTXbtc) (contract-call? .sm-vault-ststxbtc accrue)
  ERR-UNKNOWN-VAULT)))))))

(define-private (vault-system-borrow (aid uint) (amount uint) (receiver principal))
  (if (is-eq aid STX) (contract-call? .sm-vault-stx system-borrow amount receiver)
  (if (is-eq aid sBTC) (contract-call? .sm-vault-sbtc system-borrow amount receiver)
  (if (is-eq aid stSTX) (contract-call? .sm-vault-ststx system-borrow amount receiver)
  (if (is-eq aid USDC) (contract-call? .sm-vault-usdc system-borrow amount receiver)
  (if (is-eq aid USDH) (contract-call? .sm-vault-usdh system-borrow amount receiver)
  (if (is-eq aid stSTXbtc) (contract-call? .sm-vault-ststxbtc system-borrow amount receiver)
  ERR-UNKNOWN-VAULT)))))))

(define-private (vault-system-repay (aid uint) (amount uint) (ft <ft-trait>) (ft-address principal))
  (if (is-eq aid STX) (contract-call? .sm-vault-stx system-repay amount)
  (if (is-eq aid sBTC) (contract-call? .sm-vault-sbtc system-repay amount)
  (if (is-eq aid stSTX) (contract-call? .sm-vault-ststx system-repay amount)
  (if (is-eq aid USDC) (contract-call? .sm-vault-usdc system-repay amount)
  (if (is-eq aid USDH) (contract-call? .sm-vault-usdh system-repay amount)
  (if (is-eq aid stSTXbtc) (contract-call? .sm-vault-ststxbtc system-repay amount)
  ERR-UNKNOWN-VAULT)))))))

(define-private (vault-socialize-debt (aid uint) (amount uint))
  (if (is-eq aid STX) (contract-call? .sm-vault-stx socialize-debt amount)
  (if (is-eq aid sBTC) (contract-call? .sm-vault-sbtc socialize-debt amount)
  (if (is-eq aid stSTX) (contract-call? .sm-vault-ststx socialize-debt amount)
  (if (is-eq aid USDC) (contract-call? .sm-vault-usdc socialize-debt amount)
  (if (is-eq aid USDH) (contract-call? .sm-vault-usdh socialize-debt amount)
  (if (is-eq aid stSTXbtc) (contract-call? .sm-vault-ststxbtc socialize-debt amount)
  ERR-UNKNOWN-VAULT)))))))

(define-private (vault-deposit (aid uint) (amount uint) (min-out uint) (recipient principal))
  (if (is-eq aid STX) (contract-call? .sm-vault-stx deposit amount min-out recipient)
  (if (is-eq aid sBTC) (contract-call? .sm-vault-sbtc deposit amount min-out recipient)
  (if (is-eq aid stSTX) (contract-call? .sm-vault-ststx deposit amount min-out recipient)
  (if (is-eq aid USDC) (contract-call? .sm-vault-usdc deposit amount min-out recipient)
  (if (is-eq aid USDH) (contract-call? .sm-vault-usdh deposit amount min-out recipient)
  (if (is-eq aid stSTXbtc) (contract-call? .sm-vault-ststxbtc deposit amount min-out recipient)
  ERR-UNKNOWN-VAULT)))))))

(define-private (vault-redeem (aid uint) (amount uint) (min-out uint) (recipient principal))
  (if (is-eq aid STX) (contract-call? .sm-vault-stx redeem amount min-out recipient)
  (if (is-eq aid sBTC) (contract-call? .sm-vault-sbtc redeem amount min-out recipient)
  (if (is-eq aid stSTX) (contract-call? .sm-vault-ststx redeem amount min-out recipient)
  (if (is-eq aid USDC) (contract-call? .sm-vault-usdc redeem amount min-out recipient)
  (if (is-eq aid USDH) (contract-call? .sm-vault-usdh redeem amount min-out recipient)
  (if (is-eq aid stSTXbtc) (contract-call? .sm-vault-ststxbtc redeem amount min-out recipient)
  ERR-UNKNOWN-VAULT)))))))
...
(define-private (get-asset (asset principal))
  (contract-call? .sm-assets get-asset-status asset))
...
(define-private (socialize-debt-asset
  ...
  (if (not (get success acc))
      ...
      (let ((borrower (get borrower acc))
            ...
            (unwrap! (contract-call? .sm-market-vault
                                      debt-remove-scaled
                                      borrower
                                      scaled-debt
...
(define-public (collateral-add (ft <ft-trait>) (amount uint) (price-feeds (optional (list 3 (buff 8192)))))
  (let ((ft-address (contract-of ft))
    ...
    (match (contract-call? .sm-market-vault resolve-safe account)
      user-registry-data
    ...
    (let ((result (try! (contract-call? .sm-market-vault collateral-add account amount ft asset-id))))
...
(define-public (collateral-remove (ft <ft-trait>) (amount uint) (receiver (optional principal)) (price-feeds (optional (list 3 (buff 8192)))))
  (let ((ft-address (contract-of ft))
    ...
    (let ((result (try! (contract-call? .sm-market-vault
                                        collateral-remove
                                        account
                                        amount
                                        ft
...
(define-public (supply-collateral-add (ft <ft-trait>) (amount uint) (min-shares uint) (price-feeds (optional (list 3 (buff 8192)))))
  (let ((ft-address (contract-of ft))
    ...
    (let ((shares-minted 
      ...
      (if (is-eq asset-id STX) (collateral-add .sm-vault-stx shares-minted price-feeds)
      (if (is-eq asset-id sBTC) (collateral-add .sm-vault-sbtc shares-minted price-feeds)
      (if (is-eq asset-id stSTX) (collateral-add .sm-vault-ststx shares-minted price-feeds)
      (if (is-eq asset-id USDC) (collateral-add .sm-vault-usdc shares-minted price-feeds)
      (if (is-eq asset-id USDH) (collateral-add .sm-vault-usdh shares-minted price-feeds)
      (if (is-eq asset-id stSTXbtc) (collateral-add .sm-vault-ststxbtc shares-minted price-feeds)
      ERR-UNKNOWN-VAULT))))))))
)
...
(define-public (borrow (ft <ft-trait>) (amount uint) (receiver (optional principal)) (price-feeds (optional (list 3 (buff 8192)))))
  (let ((address (contract-of ft))
    ...
    (let ((scaled-debt-added (convert-to-scaled-debt asset-id amount true))
          (borrow-index (get index (unwrap-panic (get-cached-indexes asset-id)))))
      (try! (contract-call? .sm-market-vault
                            debt-add-scaled
                            account
                            scaled-debt-added
                            asset-id))
...
(define-public (liquidate
  ...
  (let (
    ...
    (let ((debt-updated (try! (contract-call? .sm-market-vault
                              debt-remove-scaled
                              borrower
                              scaled-to-remove
                              debt-aid)))
          ...
          (coll-removed (try! (contract-call? .sm-market-vault
                              collateral-remove
                              borrower
                              coll-final
                              collateral-ft
