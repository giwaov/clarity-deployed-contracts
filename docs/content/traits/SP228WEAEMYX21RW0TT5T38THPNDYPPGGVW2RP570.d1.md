...
(use-trait dlmm-pool-trait .a1.dlmm-pool-trait)
...
(define-private (fold-swap-multi
  ...
  (let (
    ...
    (swap-result (if x-for-y
                     (try! (contract-call? .c1 swap-x-for-y pool-trait x-token-trait y-token-trait active-bin-id amount))
                     (try! (contract-call? .c1 swap-y-for-x pool-trait x-token-trait y-token-trait active-bin-id amount))))
  ...
  (let (
    ...
    (if (> x-amount-for-swap u0)
        (let (
          ...
          (swap-result (try! (contract-call? .c1 swap-x-for-y pool-trait x-token-trait y-token-trait active-bin-id x-amount-for-swap)))
  ...
  (let (
    ...
    (if (> y-amount-for-swap u0)
        (let (
          ...
          (bin-id-delta (- active-bin-id expected-bin-id))
          (is-unfavorable (> bin-id-delta 0))
          (swap-result (try! (contract-call? .c1 swap-y-for-x pool-trait x-token-trait y-token-trait active-bin-id y-amount-for-swap)))
          (out (get out swap-result))
          (updated-results (unwrap! (as-max-len? (append (get results result-data) swap-result) u350) ERR_RESULTS_LIST_OVERFLOW))
