...
;; Create a new event (anyone can create)
...
;; Get latest events (returns up to 10 most recent events)
;; Pass 0 to get the latest events, or pass a starting event ID
(define-read-only (get-latest-events
  (start-from uint)
)
  (let (
    (total-events (var-get event-id-counter))
    (start-id (if (is-eq start-from u0) 
      (if (>= total-events u10) (- total-events u9) u1)
      start-from))
  )
    (if (and (>= start-id u1) (<= start-id total-events))
      (ok (list
        (map-get? events { event-id: start-id })
        (if (>= total-events (+ start-id u1)) (map-get? events { event-id: (+ start-id u1) }) none)
        (if (>= total-events (+ start-id u2)) (map-get? events { event-id: (+ start-id u2) }) none)
        (if (>= total-events (+ start-id u3)) (map-get? events { event-id: (+ start-id u3) }) none)
        (if (>= total-events (+ start-id u4)) (map-get? events { event-id: (+ start-id u4) }) none)
        (if (>= total-events (+ start-id u5)) (map-get? events { event-id: (+ start-id u5) }) none)
        (if (>= total-events (+ start-id u6)) (map-get? events { event-id: (+ start-id u6) }) none)
        (if (>= total-events (+ start-id u7)) (map-get? events { event-id: (+ start-id u7) }) none)
        (if (>= total-events (+ start-id u8)) (map-get? events { event-id: (+ start-id u8) }) none)
        (if (>= total-events (+ start-id u9)) (map-get? events { event-id: (+ start-id u9) }) none)
      ))
      (err u7001)
    )
  )
