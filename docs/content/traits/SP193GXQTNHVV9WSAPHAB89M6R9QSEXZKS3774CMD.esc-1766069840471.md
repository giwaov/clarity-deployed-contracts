...
(define-non-fungible-token ESC uint)
...
(begin
  (unwrap! (nft-mint? ESC u1 DEPLOYER) (err ERR-NFT-MINT))
  (map-set cids u1 "QmXoGcd9G8Za1w9TeuFwvNfxMyt9G3TZRVNeyYck5XVtsy")
...
(define-public (set-token-uri (token-id uint) (metadata-cid (string-ascii 256)))
  (begin
    ...
    (asserts! (is-some (nft-get-owner? ESC token-id)) (err ERR-NOT-FOUND))
...
(define-public (mint-additional (metadata-cid (string-ascii 256)))
  (let ((token-id (+ (var-get last-id) u1)))
    (asserts! (not (var-get locked)) (err ERR-CONTRACT-LOCKED))
    (try! (nft-mint? ESC token-id tx-sender))
...
(define-public (buy-in-sat (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ESC id) (err ERR-NOT-FOUND)))
...
(define-private (is-owner (token-id uint) (user principal))
  (is-eq user (unwrap! (nft-get-owner? ESC token-id) false)))

(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ESC id sender recipient)
    success
      (begin
