---
title: "Trait test-controller-hbtc-v4"
draft: true
---
```
;; @contract Controller
...
(define-constant rf .test-reserve-fund-hbtc-v4)
...
(define-public (log-reward (reward uint) (is-positive bool))
  (let (
    (state (contract-call? .test-state-hbtc-v4 get-reward-state))
    (fees (get fees state))
    ...
    (try! (contract-call? .test-hq-vaults-v4 check-is-rewarder contract-caller))
    (try! (contract-call? .test-state-hbtc-v4 check-max-reward reward))
...
(define-public (fund-transfers)
  (let (
    (pending (contract-call? .test-state-hbtc-v4 get-pending))
    ...
    (try! (contract-call? .test-hq-vaults-v4 check-is-manager contract-caller))

    (asserts! (> total-pending u0) ERR_NO_PENDING_TRANSFERS)
    (asserts! (>= total-reserve total-pending) ERR_INSUFFICIENT_FUNDS)

    (if (> pending-fees u0) (try! (contract-call? .test-reserve-hbtc-v4 transfer sbtc-token pending-fees fee-collector)) true)
    (if (> pending-rf u0) (try! (contract-call? .test-reserve-hbtc-v4 transfer sbtc-token pending-rf rf)) true)
...
(define-private (handle-profit 
  ...
  (let (
    ...
    (ok (try! (contract-call? .test-state-hbtc-v4 update-state 
      (list
...
(define-private (handle-loss-covered 
  ...
  (let (
    ...
    (if (> transfer-amount u0)
      (try! (contract-call? .test-reserve-fund-hbtc-v4 transfer sbtc-token transfer-amount reserve none))
      true
    )
