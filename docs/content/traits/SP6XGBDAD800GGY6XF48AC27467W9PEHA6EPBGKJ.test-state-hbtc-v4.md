---
title: "Trait test-state-hbtc-v4"
draft: true
---
```
...
(define-data-var staleness-window uint u300)                       ;; [50 seconds] => ~50 seconds - price staleness check
...
(define-read-only (get-share-price)
  (let (
    ...
    (total-supply (unwrap-panic (contract-call? .test-token-hbtc-v4 get-total-supply)))
  )
    (if (> total-supply u0)
...
(define-read-only (check-is-vault-active)
  (begin
    (try! (contract-call? .test-hq-vaults-v4 check-is-protocol-active))
...
(define-private (update-shares (amount uint) (is-add bool) (user principal) (current-supply uint))
  (let (
    ...
    (if is-add 
      (try! (contract-call? .test-token-hbtc-v4 mint-for-protocol amount user)) 
      (try! (contract-call? .test-token-hbtc-v4 burn-for-protocol amount user)))
...
(define-public (update-state 
  ...
  (let (
    ...
    (current-share-supply (unwrap-panic (contract-call? .test-token-hbtc-v4 get-total-supply)))
  )
    ...
    (let ((post-share-supply
      ...
      (match reward
        data (begin
          ...
          (asserts! (> (unwrap-panic (contract-call? .test-token-hbtc-v4 get-total-supply)) u0) ERR_ZERO_SUPPLY)
...
(define-public (set-fee-address (address principal))
  (begin
    (try! (contract-call? .test-hq-vaults-v4 check-is-owner contract-caller))
    (try! (contract-call? .test-hq-vaults-v4 check-is-standard address))
...
(define-public (set-fees (mgmt-fee uint) (perf-fee uint) (exit-fee uint) (express-fee uint))
  (let (
    ...
    (try! (contract-call? .test-hq-vaults-v4 check-is-admin contract-caller))
...
(define-public (set-custom-exit-fee (address principal) (exit-fee uint))
  (begin
    (try! (contract-call? .test-hq-vaults-v4 check-is-fee-setter contract-caller))
    (asserts! (<= exit-fee (get exit-fee max) ) ERR_ABOVE_MAX)
...
(define-public (disable-vault)
  (begin
    (try! (contract-call? .test-hq-vaults-v4 check-is-guardian contract-caller))
...
(define-public (activate-asset (address principal))
  (let (
    ...
    (try! (contract-call? .test-hq-vaults-v4 check-is-owner contract-caller))
    (try! (contract-call? .test-hq-vaults-v4 check-activation-delay ts))
