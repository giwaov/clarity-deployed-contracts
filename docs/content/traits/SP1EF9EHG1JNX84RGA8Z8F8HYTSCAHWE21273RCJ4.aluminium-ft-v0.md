...
(define-fungible-token ata-aluminium)

(define-data-var token-uri (optional (string-utf8 256)) (some u"https://ata-game.space/v0/token-metadata/aluminium"))

(define-map authorized-to-mint-contracts principal bool)
(map-insert authorized-to-mint-contracts .aluminium-v1 true)
...
(define-public (mint (amount uint))
  (begin
    ...
    (ft-mint? ata-aluminium amount tx-sender)
  )
)

(define-public (burn (amount uint))
  (ft-burn? ata-aluminium amount tx-sender)
)
...
(define-public (transfer
  ...
  (begin
    ...
    (try! (ft-transfer? ata-aluminium amount tx-sender recipient))
    (match memo to-print (print to-print) 0x)
    (ok true)
  )
)

(define-read-only (get-name) (ok "ATA Aluminium"))

(define-read-only (get-symbol) (ok "aLUM"))

(define-read-only (get-decimals) (ok u0))

(define-read-only (get-balance (who principal)) (ok (ft-get-balance ata-aluminium who)))

(define-read-only (get-total-supply) (ok (ft-get-supply ata-aluminium)))
