---
title: "Trait nsfw-temptations-vlogs-liana-cruzzz"
draft: true
---
```
;; nsfw-temptations-vlogs-liana-cruzzz
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token nsfw-temptations-vlogs-liana-cruzzz uint)
...
;; Internal variables
(define-data-var mint-limit uint u139)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmSXP5vjgsGU4AbkdroezTjatCVmDVZde5f4t9D4nRpXmq/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? nsfw-temptations-vlogs-liana-cruzzz next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? nsfw-temptations-vlogs-liana-cruzzz token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? nsfw-temptations-vlogs-liana-cruzzz token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? nsfw-temptations-vlogs-liana-cruzzz token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? nsfw-temptations-vlogs-liana-cruzzz id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? nsfw-temptations-vlogs-liana-cruzzz id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? nsfw-temptations-vlogs-liana-cruzzz id) (err ERR-NOT-FOUND)))
