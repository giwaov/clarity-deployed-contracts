---
title: "Trait dlmm-swap-router-v-0-2"
draft: true
---
```
;; dlmm-swap-router-v-0-2
...
(define-constant MAX_STEPS u350)
...
;; Swap through up to 350 bins in up to 5 pools
...
(define-private (fold-swap-multi
	...
	(let (
		...
		(swap-result (if x-for-y
										 (try! (contract-call? .dlmm-core-v-0-1 swap-x-for-y pool-trait x-token-trait y-token-trait active-bin-id amount))
										 (try! (contract-call? .dlmm-core-v-0-1 swap-y-for-x pool-trait x-token-trait y-token-trait active-bin-id amount))))
	...
	(let (
		...
		(if (> x-amount-for-swap u0)
				(let (
					...
					(swap-result (try! (contract-call? .dlmm-core-v-0-1 swap-x-for-y pool-trait x-token-trait y-token-trait active-bin-id x-amount-for-swap)))
	...
	(let (
		...
		(if (> y-amount-for-swap u0)
				(let (
					...
					(bin-id-delta (- active-bin-id expected-bin-id))
					(is-unfavorable (> bin-id-delta 0))
					(swap-result (try! (contract-call? .dlmm-core-v-0-1 swap-y-for-x pool-trait x-token-trait y-token-trait active-bin-id y-amount-for-swap)))
					(out (get out swap-result))
					(updated-results (unwrap! (as-max-len? (append (get results result-data) swap-result) u350) ERR_RESULTS_LIST_OVERFLOW))
