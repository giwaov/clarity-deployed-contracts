---
title: "Trait i-just-wish-to-meet-you-1765901922370"
draft: true
---
```
;; SUMAK AUDIO NFT CONTRACT
;; I_JUST_WISH_TO_MEET_YOU Audio NFT

(define-non-fungible-token I_JUST_WISH_TO_MEET_YOU uint)
...
(begin
  (unwrap! (nft-mint? I_JUST_WISH_TO_MEET_YOU u1 DEPLOYER) (err ERR-NFT-MINT))
  (map-set cids u1 "QmZHJTsjtssQL4hnh1fAmACof1bW83PZEcon6Nkf4thgKz")
...
(define-public (set-token-uri (token-id uint) (metadata-cid (string-ascii 256)))
  (begin
    ...
    (asserts! (is-some (nft-get-owner? I_JUST_WISH_TO_MEET_YOU token-id)) (err ERR-NOT-FOUND))
...
(define-public (mint-additional (metadata-cid (string-ascii 256)))
  (let ((token-id (+ (var-get last-id) u1)))
    (asserts! (not (var-get locked)) (err ERR-CONTRACT-LOCKED))
    (try! (nft-mint? I_JUST_WISH_TO_MEET_YOU token-id tx-sender))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? I_JUST_WISH_TO_MEET_YOU token-id)))
...
(define-public (buy-in-sat (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? I_JUST_WISH_TO_MEET_YOU id) (err ERR-NOT-FOUND)))
...
(define-private (is-owner (token-id uint) (user principal))
  (is-eq user (unwrap! (nft-get-owner? I_JUST_WISH_TO_MEET_YOU token-id) false)))

(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? I_JUST_WISH_TO_MEET_YOU id sender recipient)
    success
      (begin
