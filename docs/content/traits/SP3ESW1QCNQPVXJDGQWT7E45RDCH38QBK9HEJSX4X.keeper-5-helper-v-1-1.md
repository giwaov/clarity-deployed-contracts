---
title: "Trait keeper-5-helper-v-1-1"
draft: true
---
```
;; keeper-5-helper-v-1-1

;; Use all required traits
(use-trait ft-trait 'SP2AKWJYC7BNY18W1XXKPGP0YVEK63QJG4793Z2D4.sip-010-trait-ft-standard.sip-010-trait)
(use-trait keeper-action-trait .keeper-action-trait-v-1-2.keeper-action-trait)
...
(define-read-only (get-action-fee (action-trait <keeper-action-trait>))
  (ok (default-to (var-get default-action-fee) (map-get? action-fees (contract-of action-trait))))
)

;; Get fee given keeper action trait and amount
(define-read-only (get-action-fee-amount (action-trait <keeper-action-trait>) (amount uint))
  (ok (/ (* amount (default-to (var-get default-action-fee) (map-get? action-fees (contract-of action-trait)))) BPS))
)
...
(define-read-only (get-authorization-data (action-trait <keeper-action-trait>))
  (ok {
    keeper-status: (var-get keeper-status),
    action-approved: (or (default-to false (map-get? approved-actions (contract-of action-trait))) (var-get all-actions-approved)),
    action-fee: (default-to (var-get default-action-fee) (map-get? action-fees (contract-of action-trait)))
  })
)
...
(define-public (get-action-a
    ...
    (dlmm-pool-list (optional (list 26 <dlmm-pool-trait>)))
    (dlmm-staking-list (optional (list 26 <dlmm-staking-trait>)))
