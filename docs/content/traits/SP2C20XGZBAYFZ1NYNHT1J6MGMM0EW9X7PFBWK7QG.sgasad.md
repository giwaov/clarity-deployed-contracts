...
;; title: sgasad NFT Contract
;; version: 1.0
;; summary: A non-fungible token contract for sgasad NFTs
...
;; token definitions
(define-non-fungible-token sgasad uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://bafybeihoonu3pir3hmua44yfxxjjrroadzf2f5ad4gfwehfkhuuegbdllm/sgasad/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? sgasad next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? sgasad token-id) false)))

(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? sgasad id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? sgasad id) false)))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? sgasad token-id tx-sender)))
...
(define-public (buy-in-ustx (id uint))
  (let (
      (owner (unwrap! (nft-get-owner? sgasad id) (err ERR-NOT-FOUND)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? sgasad token-id)))
...
(define-public (admin-airdrop)
  (let
    ...
    (begin
      ...
      (try! (nft-mint? sgasad (+ last-nft-id u0) 'SP27ENRYMEGM9K6TVR7A8JEDTFXN5EA22KPJHVPE1))
      (map-set token-count 'SP27ENRYMEGM9K6TVR7A8JEDTFXN5EA22KPJHVPE1 (+ (get-balance 'SP27ENRYMEGM9K6TVR7A8JEDTFXN5EA22KPJHVPE1) u1))
      (try! (nft-mint? sgasad (+ last-nft-id u1) 'SP3N3B77Y7Q2KDSQS5A630AN2HGZ96P36ZJAYK1MY))
