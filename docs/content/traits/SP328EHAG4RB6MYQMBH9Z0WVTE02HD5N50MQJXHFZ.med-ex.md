...
(define-private (is-regulatory-body
    (authority principal)
    (cert-type uint)
  )
  (default-to false
    (get approved
      (map-get? regulatory-bodies {
        authority: authority,
        cert-type: cert-type,
      })
    ))
...
(define-public (register-device
  ...
  (begin
    ...
    (asserts!
      (or (is-contract-owner tx-sender) (is-eq initial-status DEVICE_STATUS_MANUFACTURED))
      ERR_UNAUTHORIZED
    )

    (map-set device-details { device-id: device-id } {
      owner: tx-sender,
...
(define-public (update-device-status
  ...
  (let ((device (unwrap! (map-get? device-details { device-id: device-id })
      ERR_INVALID_DEVICE
    )))
    (asserts! (is-valid-device-id device-id) ERR_INVALID_DEVICE)
...
(define-public (add-regulatory-body
    (authority principal)
    (cert-type uint)
  )
  (begin
...
(define-public (add-certification
  ...
  (begin
    ...
    (asserts!
      (is-none (map-get? device-certifications {
        device-id: device-id,
        cert-type: cert-type,
      }))
    ...
    (let (
      ...
      (map-set device-certifications {
        device-id: validated-device-id,
        cert-type: validated-cert-type,
      } {
        issuer: tx-sender,
        timestamp: (get-current-timestamp),
        valid: true,
...
(define-read-only (verify-certification
  ...
  (let ((certification (unwrap!
      (map-get? device-certifications {
        device-id: device-id,
        cert-type: cert-type,
      })
...
(define-read-only (get-certification-details
    (device-id uint)
    (cert-type uint)
  )
  (ok (map-get? device-certifications {
    device-id: device-id,
    cert-type: cert-type,
  }))
)
