---
title: "Trait rewards-dual-stacking-based-dollar"
draft: true
---
```
...
(define-public (initialize-contract)
  (let (
      (yield-cycle-id u2)
      (yield-snapshot-index u0)
      ;; If Yield contract is not initialized this will throw
      ;; `ERR_STX_BLOCK_NOT_VALIDATED_IN_YIELD`.
      (current-snapshot-stx-block-height (unwrap!
        (get-stacks-block-height-for-cycle-snapshot yield-cycle-id
...
(define-private (reset-state-for-cycle (yield-cycle-id-wanted uint))
  (let (
      (yield-data (contract-call? 'SP1HFCRKEJ8BYW4D0E3FAWHFDX8A25PPAA83HWWZ9.dual-stacking-v1 get-yield-cycle-data
        yield-cycle-id-wanted
      ))
      (updated-blocks-per-snapshot (unwrap! (get blocks-per-snapshot yield-data)
        ERR_CYCLE_DATA_NOT_SAVED
      ))
      (updated-snapshots-per-cycle (unwrap! (get snapshots-per-cycle yield-data)
        ERR_CYCLE_DATA_NOT_SAVED
      ))
    )
    (asserts! (is-eq contract-caller (var-get admin)) ERR_NOT_ADMIN)
...
(define-public (set-snapshot-participants-count (count uint))
  (begin
    ...
    (asserts! (is-eq -1 (var-get participants-to-snapshot))
...
(define-public (compute-current-snapshot-balances (principals (list 900 principal)))
  (begin
    (asserts! (> (var-get participants-to-snapshot) -1)
    ...
    (asserts!
      (not (is-eq (var-get participants-to-snapshot)
...
(define-private (compute-and-update-balances-one-user
  ...
  (let (
      (balance (at-block (var-get local-stx-id-header-hash)
    ...
    (if (is-eq local-current-snapshot-index
        (default-to DUMMY_SNAPSHOT_INDEX
...
(define-public (head-to-next-snapshot)
  (let (
      ...
      ;; `ERR_STX_BLOCK_NOT_VALIDATED_IN_YIELD`.
      (local-current-snapshot-index (var-get current-snapshot-index))
      (next-snapshot-stx-block-height (unwrap!
        (contract-call? 'SP1HFCRKEJ8BYW4D0E3FAWHFDX8A25PPAA83HWWZ9.dual-stacking-v1
    ...
    (asserts! (var-get is-contract-active) ERR_CONTRACT_NOT_ACTIVE)
    (asserts! (> local-participants-to-snapshot -1)
      ERR_ADD_PARTICIPANTS_COUNT
    )
    (asserts!
      (is-eq (var-get current-snapshot-participants-count)
    ...
    (ok (var-set current-snapshot-stacks-block-height
...
(define-public (conclude-cycle-snapshots)
  (let ((local-participants-to-snapshot (var-get participants-to-snapshot)))
    (asserts! (var-get is-contract-active) ERR_CONTRACT_NOT_ACTIVE)
    (asserts!
      (is-eq (+ u1 (var-get current-snapshot-index))
        (var-get snapshots-per-cycle)
      )
      ERR_NOT_ALL_SNAPSHOTS
    )
    (asserts! (not (is-eq local-participants-to-snapshot -1))
      ERR_ADD_PARTICIPANTS_COUNT
    )
    (asserts!
      (is-eq (var-get current-snapshot-participants-count)
    ...
    (asserts! (not (var-get concluded-snapshots))
...
(define-public (set-is-distribution-enabled)
  (begin
    ...
    (asserts! (can-call-set-is-distribution-enabled)
      ERR_YIELD_REWARDS_NOT_SENT_YET
    )
    (let (
        ;; Distribute the entire amount present in the smart contract.
        (pool-rewards (unwrap!
          (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token get-balance-available
      ...
      (if (is-eq pool-rewards u0)
        (var-set rewarded-count
...
(define-public (distribute-rewards (principals (list 900 principal)))
  (begin
    ...
    (if (or (is-eq (var-get rewards-to-distribute) u0) (is-eq (var-get current-cycle-total) u0))
      (begin
        (var-set rewarded-count
          (var-get current-cycle-participants-count)
        )
...
(define-private (distribute-reward-user (user principal))
  (let (
      ...
      (reward-amount (/ (* (get amount holding-state) (var-get rewards-to-distribute))
    ...
    (if (get rewarded holding-state)
      ...
      (begin
        ...
        (if (> reward-amount u0)
          (try! (as-contract?
            ((with-ft 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token "sbtc-token" reward-amount))
            (unwrap-panic (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token transfer reward-amount
              tx-sender user none
            ))
          ))
          false
...
(define-public (head-to-next-cycle)
  (let (
      (local-last-cycle-id (var-get last-cycle-id))
      (next-cycle-stx-block-height (unwrap!
        (contract-call? 'SP1HFCRKEJ8BYW4D0E3FAWHFDX8A25PPAA83HWWZ9.dual-stacking-v1
          get-stacks-block-height-for-cycle-snapshot
          (+ u1 local-last-cycle-id) u0
        )
        ERR_STX_BLOCK_NOT_VALIDATED_IN_YIELD
      ))
      (stx-block-header-hash (unwrap!
        (get-stacks-block-info? id-header-hash
...
(define-public (extract-funds-safety)
  (let ((sbtc-amount (unwrap!
    ...
    (asserts! (is-eq contract-caller (var-get admin)) ERR_NOT_ADMIN)
    (try! (as-contract? ((with-ft 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token "sbtc-token" sbtc-amount))
      (unwrap-panic (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token transfer sbtc-amount tx-sender
        (var-get admin) none
      ))
    ))
    (ok true)
...
(define-read-only (yield-contract-current-cycle-stacks-block-height)
  (get current-cycle-stacks-block-height
    (contract-call? 'SP1HFCRKEJ8BYW4D0E3FAWHFDX8A25PPAA83HWWZ9.dual-stacking-v1 cycle-data)
  )
)
...
(define-read-only (get-stacks-block-height-for-cycle-snapshot
  ...
  (contract-call? 'SP1HFCRKEJ8BYW4D0E3FAWHFDX8A25PPAA83HWWZ9.dual-stacking-v1 
    get-stacks-block-height-for-cycle-snapshot checked-cycle-id
    checked-snapshot-id
  )
)
...
(define-read-only (get-bitcoin-block-height-for-cycle-snapshot
  ...
  (contract-call? 'SP1HFCRKEJ8BYW4D0E3FAWHFDX8A25PPAA83HWWZ9.dual-stacking-v1
    get-bitcoin-block-height-for-cycle-snapshot checked-cycle-id
    checked-snapshot-id
  )
)
...
(define-read-only (can-call-set-is-distribution-enabled)
  ...
  (is-some (contract-call? 'SP1HFCRKEJ8BYW4D0E3FAWHFDX8A25PPAA83HWWZ9.dual-stacking-v1
    get-distribution-finalized-at-height (var-get last-cycle-id)
  ))
)
...
(define-read-only (stx-block-height-distribution-finalized-for-wanted-cycle (cycle-id uint))
  (get stx-block-height
    (contract-call? 'SP1HFCRKEJ8BYW4D0E3FAWHFDX8A25PPAA83HWWZ9.dual-stacking-v1
      get-distribution-finalized-at-height cycle-id
    ))
)

(define-read-only (can-call-set-snapshot-participants-count (offset-bitcoin-blocks uint))
  (begin
...
(define-read-only (state-last-operation)
  (if (and
      ...
      (is-some (contract-call? 'SP1HFCRKEJ8BYW4D0E3FAWHFDX8A25PPAA83HWWZ9.dual-stacking-v1 get-yield-cycle-data
        (+ (var-get last-cycle-id) u1)
      ))
      (is-eq (var-get current-cycle-participants-count)
        (var-get rewarded-count)
      )
  ...
  (let ((user-vaults (unwrap!
      (unwrap!
        (contract-call?
          ...
          get-vaults-by-principal address
        )
        u0
      )
      u0
    ...
    (unwrap!
      (fold calculate-user-sbtc-equivalent-balance user-vaults (ok u0))
      u0
    )
  )
)

(define-private (calculate-user-sbtc-equivalent-balance
    (vault-id uint)
...
(define-read-only (get-total-balance-at-stacks-block
  ...
  (ok (at-block
    ...
    (unwrap! (get-stacks-block-info? id-header-hash stacks-height)
      ERR_STX_BLOCK_IN_FUTURE
    )
    (get-user-total-sBTC-balance address)
  ))
