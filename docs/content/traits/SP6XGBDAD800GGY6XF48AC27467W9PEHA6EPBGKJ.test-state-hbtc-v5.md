---
title: "Trait test-state-hbtc-v5"
draft: true
---
```
...
(define-constant max {
  reward: u200,                                                    ;; [200 bps] => 0.20% - max asset reward/loss per log-reward call
  deviation: u200,                                                 ;; [200 bps] => 0.20% - max share price deviation per update
...
(define-data-var cooldown uint u60)                           ;; [60 seconds] => 1 minute - default redeem cooldown period
(define-data-var express-cooldown uint u30)                     ;; [30 seconds] => 30 seconds - express redeem cooldown period
(define-data-var update-window uint u60)                       ;; [60 seconds] => 1 minute - min time between reward updates
(define-data-var staleness-window uint u300)                       ;; [300 seconds] => 5 minutes - price staleness check
...
(define-read-only (get-share-price)
  (let (
    ...
    (total-supply (unwrap-panic (contract-call? .test-token-hbtc-v5 get-total-supply)))
  )
    (if (> total-supply u0)
...
(define-private (update-shares (amount uint) (is-add bool) (user principal) (current-supply uint))
  (let (
    ...
    (if is-add 
      (try! (contract-call? .test-token-hbtc-v5 mint-for-protocol amount user)) 
      (try! (contract-call? .test-token-hbtc-v5 burn-for-protocol amount user)))
...
(define-public (update-state 
  ...
  (let (
    ...
    (current-share-supply (unwrap-panic (contract-call? .test-token-hbtc-v5 get-total-supply)))
  )
    ...
    (let ((post-share-supply
      ...
      (match reward
        data (begin
          ...
          (asserts! (> (unwrap-panic (contract-call? .test-token-hbtc-v5 get-total-supply)) u0) ERR_ZERO_SUPPLY)
...
(define-public (set-fee-address (address principal))
  (begin
    ...
    (try! (contract-call? .test-hq-vaults-v5 check-is-standard address))
...
(define-public (set-fees (mgmt-fee uint) (perf-fee uint) (exit-fee uint) (express-fee uint))
  (let (
    ...
    (try! (contract-call? .test-hq-vaults-v5 check-is-admin contract-caller))
...
(define-public (set-custom-exit-fee (entries (list 200 { address: principal, new-exit-fee: uint })))
  (begin
    (try! (contract-call? .test-hq-vaults-v5 check-is-fee-setter contract-caller))
...
(define-public (disable-vault)
  (begin
    (try! (contract-call? .test-hq-vaults-v5 check-is-guardian contract-caller))
...
(define-public (request-new-asset (token <ft>) (price-feed-id (buff 32)) (max-slippage uint) (is-stablecoin bool))
  (let (
    ...
    (activation-ts (+ (get-current-ts) (contract-call? .test-hq-vaults-v5 get-activation-delay)))
...
(define-public (activate-asset (address principal))
  (let (
    ...
    (try! (contract-call? .test-hq-vaults-v5 check-is-owner contract-caller))
    (try! (contract-call? .test-hq-vaults-v5 check-activation-delay ts))
...
(map-set assets { address: 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token } { active: true, ts: init-ts, price-feed-id: 0xe62df6c8b4a85fe1a67db44dc12de5db330f7ac66b72dc658afedf0f4a415b43, token-base: u100000000, max-slippage: u100, is-stablecoin: false })

;; TODO set correct contracts
(map-set contracts { address: .dev-staking } { active: true, ts: init-ts })
(map-set contracts { address: .dev-staking-silo } { active: true, ts: init-ts })
(map-set contracts { address: 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.borrow-helper-v2-1-7 } { active: true, ts: init-ts })
(map-set contracts { address: .dev-xyk-swap-helper } { active: true, ts: init-ts })

;; TODO set correct contracts for HBTC protocol
...
(map-set contracts { address: .dev-minting-auto } { active: true, ts: init-ts })
...
(map-set assets { address: .dev-usdh-token } { active: true, ts: init-ts, price-feed-id: 0x00, token-base: u100000000, max-slippage: u100, is-stablecoin: true })
(map-set assets { address: .dev-susdh-token } { active: true, ts: init-ts, price-feed-id: 0x01, token-base: u100000000, max-slippage: u100, is-stablecoin: false })
(map-set assets { address: 'SPN5AKG35QZSK2M8GAMR4AFX45659RJHDW353HSG.usdh-token-v1 } { active: true, ts: init-ts, price-feed-id: 0x02, token-base: u100000000, max-slippage: u100, is-stablecoin: true })
(map-set assets { address: 'SP3Y2ZSH8P7D50B0VBTSX11S7XSG24M1VB9YFQA4K.token-aeusdc } { active: true, ts: init-ts, price-feed-id: 0xeaa020c61cc479712813461ce153894a96a6c00b21ed0cfc2798d1f9a9e9c94a, token-base: u1000000, max-slippage: u1000, is-stablecoin: true })
(map-set assets { address: 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.wstx } { active: true, ts: init-ts, price-feed-id: 0xec7a775f46379b5e943c3526b1c8d54cd49749176b0b98e02dde68d1bd335c17, token-base: u1000000, max-slippage: u1000, is-stablecoin: false })
(map-set assets { address: 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.token-stx-v-1-2 } { active: true, ts: init-ts, price-feed-id: 0xec7a775f46379b5e943c3526b1c8d54cd49749176b0b98e02dde68d1bd335c17, token-base: u1000000, max-slippage: u1000, is-stablecoin: false })
(map-set assets { address: 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token } { active: true, ts: init-ts, price-feed-id: 0xe62df6c8b4a85fe1a67db44dc12de5db330f7ac66b72dc658afedf0f4a415b43, token-base: u100000000, max-slippage: u1000, is-stablecoin: false })
...
;; @notice - Only available in dev contracts, NOT in production
