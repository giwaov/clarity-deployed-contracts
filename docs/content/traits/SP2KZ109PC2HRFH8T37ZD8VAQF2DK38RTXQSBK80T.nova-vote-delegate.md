---
title: "Trait nova-vote-delegate"
draft: true
---
```
;; nova-vote-delegate.clar
;; Allows token holders to delegate voting power to representatives in Nova Protocol.
;; CLARITY VERSION: 2
...
(define-public (delegate
        (to principal)
        (token-trait <nova-trait-governance>)
    )
    (let (
            (sender tx-sender)
...
(define-public (undelegate (token-trait <nova-trait-governance>))
    (let (
            (sender tx-sender)
