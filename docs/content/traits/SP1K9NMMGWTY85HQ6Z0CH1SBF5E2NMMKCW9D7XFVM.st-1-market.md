...
;; TRAITS
;; ============================================================================
(use-trait ft-trait .st-ft-trait.ft-trait)
(impl-trait .st-market-trait.market-trait)
...
;; -- Contract references
(define-constant ZEST-STX-WRAPPER-CONTRACT .st-wstx)
...
(define-private (check-dao-auth)
  (ok (asserts! (is-eq tx-sender .st-dao-executor) ERR-AUTH)))

;; -- Vault routing ----------------------------------------------------------

(define-private (vault-accrue (aid uint))
  (if (is-eq aid STX) (contract-call? .st-vault-stx accrue)
  (if (is-eq aid sBTC) (contract-call? .st-vault-sbtc accrue)
  (if (is-eq aid stSTX) (contract-call? .st-vault-ststx accrue)
  (if (is-eq aid USDC) (contract-call? .st-vault-usdc accrue)
  (if (is-eq aid USDH) (contract-call? .st-vault-usdh accrue)
  (if (is-eq aid stSTXbtc) (contract-call? .st-vault-ststxbtc accrue)
  ERR-UNKNOWN-VAULT)))))))

(define-private (vault-system-borrow (aid uint) (amount uint) (receiver principal))
  (if (is-eq aid STX) (contract-call? .st-vault-stx system-borrow amount receiver)
  (if (is-eq aid sBTC) (contract-call? .st-vault-sbtc system-borrow amount receiver)
  (if (is-eq aid stSTX) (contract-call? .st-vault-ststx system-borrow amount receiver)
  (if (is-eq aid USDC) (contract-call? .st-vault-usdc system-borrow amount receiver)
  (if (is-eq aid USDH) (contract-call? .st-vault-usdh system-borrow amount receiver)
  (if (is-eq aid stSTXbtc) (contract-call? .st-vault-ststxbtc system-borrow amount receiver)
  ERR-UNKNOWN-VAULT)))))))

(define-private (vault-system-repay (aid uint) (amount uint) (ft <ft-trait>) (ft-address principal))
  (if (is-eq aid STX) (contract-call? .st-vault-stx system-repay amount)
  (if (is-eq aid sBTC) (contract-call? .st-vault-sbtc system-repay amount)
  (if (is-eq aid stSTX) (contract-call? .st-vault-ststx system-repay amount)
  (if (is-eq aid USDC) (contract-call? .st-vault-usdc system-repay amount)
  (if (is-eq aid USDH) (contract-call? .st-vault-usdh system-repay amount)
  (if (is-eq aid stSTXbtc) (contract-call? .st-vault-ststxbtc system-repay amount)
  ERR-UNKNOWN-VAULT)))))))

(define-private (vault-socialize-debt (aid uint) (amount uint))
  (if (is-eq aid STX) (contract-call? .st-vault-stx socialize-debt amount)
  (if (is-eq aid sBTC) (contract-call? .st-vault-sbtc socialize-debt amount)
  (if (is-eq aid stSTX) (contract-call? .st-vault-ststx socialize-debt amount)
  (if (is-eq aid USDC) (contract-call? .st-vault-usdc socialize-debt amount)
  (if (is-eq aid USDH) (contract-call? .st-vault-usdh socialize-debt amount)
  (if (is-eq aid stSTXbtc) (contract-call? .st-vault-ststxbtc socialize-debt amount)
  ERR-UNKNOWN-VAULT)))))))

(define-private (vault-deposit (aid uint) (amount uint) (min-out uint) (recipient principal))
  (if (is-eq aid STX) (contract-call? .st-vault-stx deposit amount min-out recipient)
  (if (is-eq aid sBTC) (contract-call? .st-vault-sbtc deposit amount min-out recipient)
  (if (is-eq aid stSTX) (contract-call? .st-vault-ststx deposit amount min-out recipient)
  (if (is-eq aid USDC) (contract-call? .st-vault-usdc deposit amount min-out recipient)
  (if (is-eq aid USDH) (contract-call? .st-vault-usdh deposit amount min-out recipient)
  (if (is-eq aid stSTXbtc) (contract-call? .st-vault-ststxbtc deposit amount min-out recipient)
  ERR-UNKNOWN-VAULT)))))))

(define-private (vault-redeem (aid uint) (amount uint) (min-out uint) (recipient principal))
  (if (is-eq aid STX) (contract-call? .st-vault-stx redeem amount min-out recipient)
  (if (is-eq aid sBTC) (contract-call? .st-vault-sbtc redeem amount min-out recipient)
  (if (is-eq aid stSTX) (contract-call? .st-vault-ststx redeem amount min-out recipient)
  (if (is-eq aid USDC) (contract-call? .st-vault-usdc redeem amount min-out recipient)
  (if (is-eq aid USDH) (contract-call? .st-vault-usdh redeem amount min-out recipient)
  (if (is-eq aid stSTXbtc) (contract-call? .st-vault-ststxbtc redeem amount min-out recipient)
  ERR-UNKNOWN-VAULT)))))))
...
(define-private (accrue-collateral-asset
  ...
  (let ((aid (get aid coll-entry)))
    ...
    (if (is-ztoken aid)
        ...
        ;; zSTX(5)->STX(0), zsBTC(6)->sBTC(1), zstSTX(7)->stSTX(2), zUSDC(8)->USDC(3), zUSDH(9)->USDH(4), zstSTXbtc (11) -> stSTXbtc(10)
...
(define-private (get-asset (asset principal))
  (contract-call? .st-assets get-asset-status asset))
...
(define-private (socialize-debt-asset
  ...
  (if (not (get success acc))
      ...
      (let ((borrower (get borrower acc))
            ...
            (unwrap! (contract-call? .st-market-vault
                                      debt-remove-scaled
                                      borrower
                                      scaled-debt
...
(define-public (collateral-add (ft <ft-trait>) (amount uint) (price-feeds (optional (list 3 (buff 8192)))))
  (let ((ft-address (contract-of ft))
    ...
    (match (contract-call? .st-market-vault resolve-safe account)
      user-registry-data
    ...
    (let ((result (try! (contract-call? .st-market-vault collateral-add account amount ft asset-id))))
...
(define-public (collateral-remove (ft <ft-trait>) (amount uint) (receiver (optional principal)) (price-feeds (optional (list 3 (buff 8192)))))
  (let ((ft-address (contract-of ft))
    ...
    (let ((result (try! (contract-call? .st-market-vault
                                        collateral-remove
                                        account
                                        amount
                                        ft
...
(define-public (supply-collateral-add (ft <ft-trait>) (amount uint) (min-shares uint) (price-feeds (optional (list 3 (buff 8192)))))
  (let ((ft-address (contract-of ft))
    ...
    (let ((shares-minted 
      ...
      (if (is-eq asset-id STX) (collateral-add .st-vault-stx shares-minted price-feeds)
      (if (is-eq asset-id sBTC) (collateral-add .st-vault-sbtc shares-minted price-feeds)
      (if (is-eq asset-id stSTX) (collateral-add .st-vault-ststx shares-minted price-feeds)
      (if (is-eq asset-id USDC) (collateral-add .st-vault-usdc shares-minted price-feeds)
      (if (is-eq asset-id USDH) (collateral-add .st-vault-usdh shares-minted price-feeds)
      (if (is-eq asset-id stSTXbtc) (collateral-add .st-vault-ststxbtc shares-minted price-feeds)
      ERR-UNKNOWN-VAULT))))))))
)
...
(define-public (collateral-remove-redeem (ft <ft-trait>) (amount uint) (min-underlying uint) (receiver (optional principal)) (price-feeds (optional (list 3 (buff 8192)))))
  (let ((ft-address (contract-of ft))
    ...
    (asserts! (or (<= underlying-id USDH) (is-eq underlying-id stSTXbtc)) ERR-UNKNOWN-VAULT)
...
(define-public (borrow (ft <ft-trait>) (amount uint) (receiver (optional principal)) (price-feeds (optional (list 3 (buff 8192)))))
  (let ((address (contract-of ft))
    ...
    (let ((scaled-debt-added (convert-to-scaled-debt asset-id amount true))
          (borrow-index (get index (unwrap-panic (get-cached-indexes asset-id)))))
      (try! (contract-call? .st-market-vault
                            debt-add-scaled
                            account
                            scaled-debt-added
                            asset-id))
...
(define-public (liquidate
  ...
  (let (
    ...
    (let ((debt-updated (try! (contract-call? .st-market-vault
                              debt-remove-scaled
                              borrower
                              scaled-to-remove
                              debt-aid)))
          ...
          (coll-removed (try! (contract-call? .st-market-vault
                              collateral-remove
                              borrower
                              coll-final
                              collateral-ft
