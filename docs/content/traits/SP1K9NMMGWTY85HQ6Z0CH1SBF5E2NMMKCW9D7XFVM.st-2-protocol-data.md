---
title: "Trait st-2-protocol-data"
draft: true
---
```
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;; -- Iteration helpers
(define-constant VAULT-IDS (list u0 u1 u2 u3 u4 u10))
...
(define-constant UNDERLYING-STX  .st-wstx)
...
(define-private (get-ststx-ratio)
  ;; TODO: Replace with actual stSTX data core call when available
  ;; (contract-call? STSTX-DATA-CORE get-stx-per-ststx STSTX-RESERVE)
  STSTX-RATIO-DECIMALS)  ;; Default 1:1 ratio

;; -- Vault routing: get interest rate --------------------------------------

(define-private (get-vault-interest-rate (vid uint))
  (if (is-eq vid STX) (unwrap-panic (contract-call? .st-vault-stx get-interest-rate))
  (if (is-eq vid sBTC) (unwrap-panic (contract-call? .st-vault-sbtc get-interest-rate))
  (if (is-eq vid stSTX) (unwrap-panic (contract-call? .st-vault-ststx get-interest-rate))
  (if (is-eq vid USDC) (unwrap-panic (contract-call? .st-vault-usdc get-interest-rate))
  (if (is-eq vid USDH) (unwrap-panic (contract-call? .st-vault-usdh get-interest-rate))
  (if (is-eq vid stSTXbtc) (unwrap-panic (contract-call? .st-vault-ststxbtc get-interest-rate))
  u0)))))))

;; -- Vault routing: get utilization -----------------------------------------

(define-private (get-vault-utilization (vid uint))
  (if (is-eq vid STX) (unwrap-panic (contract-call? .st-vault-stx get-utilization))
  (if (is-eq vid sBTC) (unwrap-panic (contract-call? .st-vault-sbtc get-utilization))
  (if (is-eq vid stSTX) (unwrap-panic (contract-call? .st-vault-ststx get-utilization))
  (if (is-eq vid USDC) (unwrap-panic (contract-call? .st-vault-usdc get-utilization))
  (if (is-eq vid USDH) (unwrap-panic (contract-call? .st-vault-usdh get-utilization))
  (if (is-eq vid stSTXbtc) (unwrap-panic (contract-call? .st-vault-ststxbtc get-utilization))
  u0)))))))

;; -- Vault routing: get fee reserve -----------------------------------------

(define-private (get-vault-fee-reserve (vid uint))
  (if (is-eq vid STX) (unwrap-panic (contract-call? .st-vault-stx get-fee-reserve))
  (if (is-eq vid sBTC) (unwrap-panic (contract-call? .st-vault-sbtc get-fee-reserve))
  (if (is-eq vid stSTX) (unwrap-panic (contract-call? .st-vault-ststx get-fee-reserve))
  (if (is-eq vid USDC) (unwrap-panic (contract-call? .st-vault-usdc get-fee-reserve))
  (if (is-eq vid USDH) (unwrap-panic (contract-call? .st-vault-usdh get-fee-reserve))
  (if (is-eq vid stSTXbtc) (unwrap-panic (contract-call? .st-vault-ststxbtc get-fee-reserve))
  u0)))))))

;; -- Vault routing: get total assets ----------------------------------------

(define-private (get-vault-total-assets (vid uint))
  (if (is-eq vid STX) (unwrap-panic (contract-call? .st-vault-stx get-total-assets))
  (if (is-eq vid sBTC) (unwrap-panic (contract-call? .st-vault-sbtc get-total-assets))
  (if (is-eq vid stSTX) (unwrap-panic (contract-call? .st-vault-ststx get-total-assets))
  (if (is-eq vid USDC) (unwrap-panic (contract-call? .st-vault-usdc get-total-assets))
  (if (is-eq vid USDH) (unwrap-panic (contract-call? .st-vault-usdh get-total-assets))
  (if (is-eq vid stSTXbtc) (unwrap-panic (contract-call? .st-vault-ststxbtc get-total-assets))
  u0)))))))

;; -- Vault routing: get debt ------------------------------------------------

(define-private (get-vault-debt (vid uint))
  (if (is-eq vid STX) (unwrap-panic (contract-call? .st-vault-stx get-debt))
  (if (is-eq vid sBTC) (unwrap-panic (contract-call? .st-vault-sbtc get-debt))
  (if (is-eq vid stSTX) (unwrap-panic (contract-call? .st-vault-ststx get-debt))
  (if (is-eq vid USDC) (unwrap-panic (contract-call? .st-vault-usdc get-debt))
  (if (is-eq vid USDH) (unwrap-panic (contract-call? .st-vault-usdh get-debt))
  (if (is-eq vid stSTXbtc) (unwrap-panic (contract-call? .st-vault-ststxbtc get-debt))
  u0)))))))

;; -- Vault routing: get total supply ----------------------------------------

(define-private (get-vault-total-supply (vid uint))
  (if (is-eq vid STX) (unwrap-panic (contract-call? .st-vault-stx get-total-supply))
  (if (is-eq vid sBTC) (unwrap-panic (contract-call? .st-vault-sbtc get-total-supply))
  (if (is-eq vid stSTX) (unwrap-panic (contract-call? .st-vault-ststx get-total-supply))
  (if (is-eq vid USDC) (unwrap-panic (contract-call? .st-vault-usdc get-total-supply))
  (if (is-eq vid USDH) (unwrap-panic (contract-call? .st-vault-usdh get-total-supply))
  (if (is-eq vid stSTXbtc) (unwrap-panic (contract-call? .st-vault-ststxbtc get-total-supply))
  u0)))))))

;; -- Vault routing: get borrow index ----------------------------------------

(define-private (get-vault-borrow-index (vid uint))
  (if (is-eq vid STX) (unwrap-panic (contract-call? .st-vault-stx get-index))
  (if (is-eq vid sBTC) (unwrap-panic (contract-call? .st-vault-sbtc get-index))
  (if (is-eq vid stSTX) (unwrap-panic (contract-call? .st-vault-ststx get-index))
  (if (is-eq vid USDC) (unwrap-panic (contract-call? .st-vault-usdc get-index))
  (if (is-eq vid USDH) (unwrap-panic (contract-call? .st-vault-usdh get-index))
  (if (is-eq vid stSTXbtc) (unwrap-panic (contract-call? .st-vault-ststxbtc get-index))
  INDEX-PRECISION)))))))

;; -- Vault routing: get liquidity index -------------------------------------

(define-private (get-vault-liquidity-index (vid uint))
  (if (is-eq vid STX) (unwrap-panic (contract-call? .st-vault-stx get-liquidity-index))
  (if (is-eq vid sBTC) (unwrap-panic (contract-call? .st-vault-sbtc get-liquidity-index))
  (if (is-eq vid stSTX) (unwrap-panic (contract-call? .st-vault-ststx get-liquidity-index))
  (if (is-eq vid USDC) (unwrap-panic (contract-call? .st-vault-usdc get-liquidity-index))
  (if (is-eq vid USDH) (unwrap-panic (contract-call? .st-vault-usdh get-liquidity-index))
  (if (is-eq vid stSTXbtc) (unwrap-panic (contract-call? .st-vault-ststxbtc get-liquidity-index))
  INDEX-PRECISION)))))))

;; -- Vault routing: get cap supply ------------------------------------------

(define-private (get-vault-cap-supply (vid uint))
  (if (is-eq vid STX) (unwrap-panic (contract-call? .st-vault-stx get-cap-supply))
  (if (is-eq vid sBTC) (unwrap-panic (contract-call? .st-vault-sbtc get-cap-supply))
  (if (is-eq vid stSTX) (unwrap-panic (contract-call? .st-vault-ststx get-cap-supply))
  (if (is-eq vid USDC) (unwrap-panic (contract-call? .st-vault-usdc get-cap-supply))
  (if (is-eq vid USDH) (unwrap-panic (contract-call? .st-vault-usdh get-cap-supply))
  (if (is-eq vid stSTXbtc) (unwrap-panic (contract-call? .st-vault-ststxbtc get-cap-supply))
  u0)))))))

;; -- Vault routing: get cap debt --------------------------------------------

(define-private (get-vault-cap-debt (vid uint))
  (if (is-eq vid STX) (unwrap-panic (contract-call? .st-vault-stx get-cap-debt))
  (if (is-eq vid sBTC) (unwrap-panic (contract-call? .st-vault-sbtc get-cap-debt))
  (if (is-eq vid stSTX) (unwrap-panic (contract-call? .st-vault-ststx get-cap-debt))
  (if (is-eq vid USDC) (unwrap-panic (contract-call? .st-vault-usdc get-cap-debt))
  (if (is-eq vid USDH) (unwrap-panic (contract-call? .st-vault-usdh get-cap-debt))
  (if (is-eq vid stSTXbtc) (unwrap-panic (contract-call? .st-vault-ststxbtc get-cap-debt))
  u0)))))))

;; -- Vault routing: get last update -----------------------------------------

(define-private (get-vault-last-update (vid uint))
  (if (is-eq vid STX) (unwrap-panic (contract-call? .st-vault-stx get-last-update))
  (if (is-eq vid sBTC) (unwrap-panic (contract-call? .st-vault-sbtc get-last-update))
  (if (is-eq vid stSTX) (unwrap-panic (contract-call? .st-vault-ststx get-last-update))
  (if (is-eq vid USDC) (unwrap-panic (contract-call? .st-vault-usdc get-last-update))
  (if (is-eq vid USDH) (unwrap-panic (contract-call? .st-vault-usdh get-last-update))
  (if (is-eq vid stSTXbtc) (unwrap-panic (contract-call? .st-vault-ststxbtc get-last-update))
  u0)))))))

;; -- Vault routing: get points util -----------------------------------------

(define-private (get-vault-points-util (vid uint))
  (if (is-eq vid STX) (unwrap-panic (contract-call? .st-vault-stx get-points-util))
  (if (is-eq vid sBTC) (unwrap-panic (contract-call? .st-vault-sbtc get-points-util))
  (if (is-eq vid stSTX) (unwrap-panic (contract-call? .st-vault-ststx get-points-util))
  (if (is-eq vid USDC) (unwrap-panic (contract-call? .st-vault-usdc get-points-util))
  (if (is-eq vid USDH) (unwrap-panic (contract-call? .st-vault-usdh get-points-util))
  (if (is-eq vid stSTXbtc) (unwrap-panic (contract-call? .st-vault-ststxbtc get-points-util))
  (list u0 u0 u0 u0 u0 u0 u0 u0))))))))

;; -- Vault routing: get points rate -----------------------------------------

(define-private (get-vault-points-rate (vid uint))
  (if (is-eq vid STX) (unwrap-panic (contract-call? .st-vault-stx get-points-rate))
  (if (is-eq vid sBTC) (unwrap-panic (contract-call? .st-vault-sbtc get-points-rate))
  (if (is-eq vid stSTX) (unwrap-panic (contract-call? .st-vault-ststx get-points-rate))
  (if (is-eq vid USDC) (unwrap-panic (contract-call? .st-vault-usdc get-points-rate))
  (if (is-eq vid USDH) (unwrap-panic (contract-call? .st-vault-usdh get-points-rate))
  (if (is-eq vid stSTXbtc) (unwrap-panic (contract-call? .st-vault-ststxbtc get-points-rate))
  (list u0 u0 u0 u0 u0 u0 u0 u0))))))))
...
(define-private (get-vault-underlying-balance (vid uint) (account principal))
  (if (is-eq vid STX)
      (let ((shares (unwrap-panic (contract-call? .st-vault-stx get-balance account))))
        (unwrap-panic (contract-call? .st-vault-stx convert-to-assets shares)))
  (if (is-eq vid sBTC)
      (let ((shares (unwrap-panic (contract-call? .st-vault-sbtc get-balance account))))
        (unwrap-panic (contract-call? .st-vault-sbtc convert-to-assets shares)))
  (if (is-eq vid stSTX)
      (let ((shares (unwrap-panic (contract-call? .st-vault-ststx get-balance account))))
        (unwrap-panic (contract-call? .st-vault-ststx convert-to-assets shares)))
  (if (is-eq vid USDC)
      (let ((shares (unwrap-panic (contract-call? .st-vault-usdc get-balance account))))
        (unwrap-panic (contract-call? .st-vault-usdc convert-to-assets shares)))
  (if (is-eq vid USDH)
      (let ((shares (unwrap-panic (contract-call? .st-vault-usdh get-balance account))))
        (unwrap-panic (contract-call? .st-vault-usdh convert-to-assets shares)))
  (if (is-eq vid stSTXbtc)
      (let ((shares (unwrap-panic (contract-call? .st-vault-ststxbtc get-balance account))))
        (unwrap-panic (contract-call? .st-vault-ststxbtc convert-to-assets shares)))
  u0)))))))
...
(define-private (get-vault-available-liquidity (vid uint))
  (if (is-eq vid STX) (contract-call? .st-vault-stx get-available-assets)
  (if (is-eq vid sBTC) (contract-call? .st-vault-sbtc get-available-assets)
  (if (is-eq vid stSTX) (contract-call? .st-vault-ststx get-available-assets)
  (if (is-eq vid USDC) (contract-call? .st-vault-usdc get-available-assets)
  (if (is-eq vid USDH) (contract-call? .st-vault-usdh get-available-assets)
  (if (is-eq vid stSTXbtc) (contract-call? .st-vault-ststxbtc get-available-assets)
  u0)))))))
...
(define-private (iter-build-asset (id uint) (acc { max-id: uint, result: (list 128 {
  ...
  (let ((max-id (get max-id acc)))
    (if (>= id max-id)
        acc
        (let ((asset-status (unwrap-panic (contract-call? .st-assets get-status id)))
              (entry {
                id: id,
...
(define-private (iter-build-egroup (id uint) (acc { max-id: uint, result: (list 128 {
  ...
  (let ((max-id (get max-id acc)))
    (if (>= id max-id)
        acc
        (let ((egroup-data (contract-call? .st-egroup lookup id))
              (entry {
                id: id,
...
(define-read-only (get-user-position (account principal))
  (let ((enabled-mask (contract-call? .st-assets get-bitmap)))
    (match (contract-call? .st-market-vault get-position account enabled-mask)
      position
        (let ((mask (get mask position))
              ...
              (egroup-result (contract-call? .st-egroup resolve mask)))
          (match egroup-result
            egroup
...
(define-private (build-debt-entry (debt-entry { aid: uint, scaled: uint }))
  (let ((aid (get aid debt-entry))
        ...
        (asset-status (unwrap-panic (contract-call? .st-assets get-status aid)))
...
(define-private (sum-collateral-usd (entry { aid: uint, amount: uint }) (acc uint))
  (let ((aid (get aid entry))
        ...
        (asset-data (unwrap-panic (contract-call? .st-assets get-status aid)))
...
(define-private (get-asset-price (aid uint))
  ...
  ;; zstSTXbtc - STX price x liquidity index
...
(define-read-only (get-all-assets)
  (let ((nonce (unwrap-panic (contract-call? .st-assets get-nonce)))
...
(define-read-only (get-reserve (vid uint))
  (if (or (<= vid USDH) (is-eq vid stSTXbtc))
...
(define-read-only (get-all-egroups)
  (let ((nonce (unwrap-panic (contract-call? .st-egroup get-nonce)))
        (init { max-id: nonce, result: (list) })
...
(define-read-only (get-protocol-summary)
  (let ((stx-data (build-reserve-data STX))
    ...
    (ok {
      ...
      asset-count: (unwrap-panic (contract-call? .st-assets get-nonce)),
      egroup-count: (unwrap-panic (contract-call? .st-egroup get-nonce))
    })))
...
(define-read-only (get-supplies-user (account principal))
  (let ((enabled-mask (contract-call? .st-assets get-bitmap))
        ...
        (vault-stx-shares (unwrap-panic (contract-call? .st-vault-stx get-balance account)))
        (vault-sbtc-shares (unwrap-panic (contract-call? .st-vault-sbtc get-balance account)))
        (vault-ststx-shares (unwrap-panic (contract-call? .st-vault-ststx get-balance account)))
        (vault-usdc-shares (unwrap-panic (contract-call? .st-vault-usdc get-balance account)))
        (vault-usdh-shares (unwrap-panic (contract-call? .st-vault-usdh get-balance account)))
        (vault-ststxbtc-shares (unwrap-panic (contract-call? .st-vault-ststxbtc get-balance account))))
...
(define-read-only (get-all-vault-ratios)
  (ok {
    stx: {
      vault-id: STX,
      shares-to-assets: (unwrap-panic (contract-call? .st-vault-stx convert-to-assets u1000000)),
      underlying: UNDERLYING-STX
    },
    sbtc: {
      vault-id: sBTC,
      shares-to-assets: (unwrap-panic (contract-call? .st-vault-sbtc convert-to-assets u100000000)),
      underlying: UNDERLYING-SBTC
    },
    ststx: {
      vault-id: stSTX,
      shares-to-assets: (unwrap-panic (contract-call? .st-vault-ststx convert-to-assets u1000000)),
      underlying: UNDERLYING-STSTX
    },
    usdc: {
      vault-id: USDC,
      shares-to-assets: (unwrap-panic (contract-call? .st-vault-usdc convert-to-assets u1000000)),
      underlying: UNDERLYING-USDC
    },
    usdh: {
      vault-id: USDH,
      shares-to-assets: (unwrap-panic (contract-call? .st-vault-usdh convert-to-assets u1000000)),
      underlying: UNDERLYING-USDH
    },
    ststxbtc: {
      vault-id: stSTXbtc,
      shares-to-assets: (unwrap-panic (contract-call? .st-vault-ststxbtc convert-to-assets u1000000)),
...
(define-read-only (get-market-vault-balances)
  (ok {
    ...
    underlying: {
      wstx: (unwrap-panic (contract-call? .st-wstx get-balance .st-market-vault)),
      sbtc: (unwrap-panic (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token get-balance .st-market-vault)),
      ststx: (unwrap-panic (contract-call? 'SP4SZE494VC2YC5JYG7AYFQ44F5Q4PYV7DVMDPBG.ststx-token get-balance .st-market-vault)),
      usdc: (unwrap-panic (contract-call? 'SP120SBRBQJ00MCWS7TM5R8WJNTTKD5K0HFRC2CNE.usdcx get-balance .st-market-vault)),
      usdh: (unwrap-panic (contract-call? 'SPN5AKG35QZSK2M8GAMR4AFX45659RJHDW353HSG.usdh-token-v1 get-balance .st-market-vault)),
      ststxbtc: (unwrap-panic (contract-call? 'SP4SZE494VC2YC5JYG7AYFQ44F5Q4PYV7DVMDPBG.ststxbtc-token-v2 get-balance .st-market-vault))
    },
    ;; ZToken vault shares held by market-vault
    ;; These are user collateral tracked in market-vault's position maps
    ztokens: {
      vault-stx: (unwrap-panic (contract-call? .st-vault-stx get-balance .st-market-vault)),
      vault-sbtc: (unwrap-panic (contract-call? .st-vault-sbtc get-balance .st-market-vault)),
      vault-ststx: (unwrap-panic (contract-call? .st-vault-ststx get-balance .st-market-vault)),
      vault-usdc: (unwrap-panic (contract-call? .st-vault-usdc get-balance .st-market-vault)),
      vault-usdh: (unwrap-panic (contract-call? .st-vault-usdh get-balance .st-market-vault)),
      vault-ststxbtc: (unwrap-panic (contract-call? .st-vault-ststxbtc get-balance .st-market-vault))
    }
  }))
...
(define-read-only (get-user-sbtc-balances (account principal))
  (let (
    (vault-shares (unwrap-panic (contract-call? .st-vault-sbtc get-balance account)))
    (vault-underlying (unwrap-panic (contract-call? .st-vault-sbtc convert-to-assets vault-shares)))
    
    (market-sbtc
      (match (contract-call? .st-market-vault get-position account u2)  ;; mask = 2^1 = 2
        position
