...
(define-data-var user-mint-amount uint u10000000000000)
...
(define-read-only (get-balance (account principal))
  (ok (ft-get-balance tUSDH account))
)

(define-read-only (get-balance-simple (account principal))
  (ft-get-balance tUSDH account)
)

(define-read-only (get-total-supply)
  (ok (ft-get-supply tUSDH))
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (match (ft-transfer? tUSDH amount sender recipient)
      response (begin
        (print memo)
        (ok response)
      )
...
(define-public (mint)
  (begin
    ...
    (ft-mint? tUSDH (var-get user-mint-amount) tx-sender)
  )
)
...
(define-public (admin-mint (amount uint) (recipient principal))
  (begin
    (asserts! (is-eq tx-sender CONTRACT_DEPLOYER) ERR_NOT_AUTHORIZED)
    (ft-mint? tUSDH amount recipient)
  )
)

(define-public (burn (amount uint))
  (begin
    (ft-burn? tUSDH amount tx-sender)
  )
)

(ft-mint? tUSDH DEPLOY_MINT_AMOUNT CONTRACT_DEPLOYER)
```
