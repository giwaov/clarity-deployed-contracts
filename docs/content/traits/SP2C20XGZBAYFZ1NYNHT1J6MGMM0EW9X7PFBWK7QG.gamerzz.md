...
;; token definition
(define-non-fungible-token gamerzz uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://bafkreia3zkxa2m6z23dywikhuvyjqhq6ny4ybb5xhk4l6xcprhjnirkuqi")
...
(define-public (mint)
  (let ((token-id (var-get next-id)) (caller tx-sender))
    ...
    (try! (nft-mint? gamerzz token-id caller))
...
(define-public (mint-to (recipient principal))
  (let ((token-id (var-get next-id)) (caller (get-standard-caller)))
    ...
    (try! (nft-mint? gamerzz token-id recipient))
...
(define-private (airdrop-mint (recipient principal) (id uint))
  (if (is-some (map-get? has-minted recipient))
    id
    (match (nft-mint? gamerzz id recipient)
...
(define-public (burn (id uint))
  (let ((owner (unwrap! (nft-get-owner? gamerzz id) ERR-TOKEN-NOT-FOUND)))
    (asserts! (is-eq tx-sender owner) ERR-NOT-AUTHORIZED)
    (try! (nft-burn? gamerzz id owner))
    (ok true)
  )
)
