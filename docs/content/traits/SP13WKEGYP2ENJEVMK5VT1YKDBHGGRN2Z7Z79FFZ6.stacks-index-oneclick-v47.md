---
title: "Trait stacks-index-oneclick-v47"
draft: true
---
```
;; =====================================================================
...
;; - BITCOIN_MAXI/DEFI_YIELD: STX, sBTC, stSTX
;; - MEME_HUNTER: WELSH, LEO, DOG, DROID
;; =====================================================================

(define-public (sell-portfolio
    ;; STX -> USDCx (via v-1-2 swap-helper-a reversed)
    (stx-amount uint)
    (stx-min-out uint)
    (stx-xyk-token-a <ft-trait>)
    (stx-xyk-token-b <ft-trait>)
    ...
    (sbtc-ss-token-b <ft-trait>)
    (sbtc-ss-pool <stableswap-pool-trait>)
    ;; stSTX -> USDCx (two-step: stSTX -> STX, then STX -> USDCx)
    (ststx-amount uint)
    (ststx-stx-min-out uint)
    ...
    (ststx-ss-token-b <ft-trait>)
    (ststx-ss-pool <stableswap-pool-trait>)
    ;; WELSH -> USDCx (via v-1-5 swap-helper-a reversed)
    (welsh-amount uint)
    (welsh-min-out uint)
    (welsh-xyk-token-a <ft-trait>)
    (welsh-xyk-token-b <ft-trait>)
    ...
    (welsh-ss14-token-b <ft-trait>)
    (welsh-ss14-pool <stableswap-pool-trait>)
    ;; LEO -> USDCx (via v-1-5 swap-helper-a reversed)
    (leo-amount uint)
    (leo-min-out uint)
    (leo-xyk-token-a <ft-trait>)
    (leo-xyk-token-b <ft-trait>)
  ...
  (begin
    ;; Sell STX -> USDCx (reversed route: XYK first, then stableswap)
    (if (> stx-amount u0)
      (begin
        (unwrap! (contract-call? 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.router-stableswap-xyk-multihop-v-1-2 swap-helper-a
    ...
    ;; Sell sBTC -> USDCx (reversed route)
    (if (> sbtc-amount u0)
      (begin
        (unwrap! (contract-call? 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.router-stableswap-xyk-multihop-v-1-2 swap-helper-b
    ...
    ;; Sell stSTX -> USDCx (two-step: stSTX -> STX, then STX -> USDCx)
    (if (> ststx-amount u0)
      (let
        (
          ;; Step 1: stSTX -> STX via StackingDAO stableswap
          (stx-received (unwrap! (contract-call? 'SPQC38PW542EQJ5M11CR25P7BS1CA6QT4TBXGB3M.stableswap-stx-ststx-v-1-2 swap-y-for-x
            'SP4SZE494VC2YC5JYG7AYFQ44F5Q4PYV7DVMDPBG.ststx-token
            'SPQC38PW542EQJ5M11CR25P7BS1CA6QT4TBXGB3M.stx-ststx-lp-token-v-1-2
            ststx-amount
            ststx-stx-min-out
        ...
        ;; Step 2: STX -> USDCx via router (reversed)
        (unwrap! (contract-call? 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.router-stableswap-xyk-multihop-v-1-2 swap-helper-a
    ...
    ;; Sell WELSH -> USDCx (via v-1-5 swap-helper-a reversed)
    (if (> welsh-amount u0)
      (begin
        (unwrap! (contract-call? 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.router-stableswap-xyk-multihop-v-1-5 swap-helper-a
    ...
    ;; Sell LEO -> USDCx (via v-1-5 swap-helper-a reversed)
    (if (> leo-amount u0)
      (begin
        (unwrap! (contract-call? 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.router-stableswap-xyk-multihop-v-1-5 swap-helper-a
    ...
    ;; Sell DOG -> USDCx (via v-1-2 swap-helper-c reversed)
    (if (> dog-amount u0)
      (begin
        (unwrap! (contract-call? 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.router-stableswap-xyk-multihop-v-1-2 swap-helper-c
    ...
    ;; Sell DROID -> USDCx (via v-1-2 swap-helper-c reversed)
    (if (> droid-amount u0)
      (begin
        (unwrap! (contract-call? 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.router-stableswap-xyk-multihop-v-1-2 swap-helper-c
    ...
    (print { event: "sell-portfolio", seller: tx-sender, stx: stx-amount, sbtc: sbtc-amount, ststx: ststx-amount, welsh: welsh-amount, leo: leo-amount, dog: dog-amount, droid: droid-amount })
    (ok { sold-stx: stx-amount, sold-sbtc: sbtc-amount, sold-ststx: ststx-amount, sold-welsh: welsh-amount, sold-leo: leo-amount, sold-dog: dog-amount, sold-droid: droid-amount })
  )
)
