...
(define-public (stake (token-id uint))
  (begin
    ...
    (try! (contract-call? .public-mint-nft transfer token-id tx-sender (as-contract tx-sender)))
    (map-set staked
      { token-id: token-id }
  ...
  (let ((stake-data (unwrap! (map-get? staked { token-id: token-id }) err-not-staked)))
    (begin
      ...
      (let (
        ...
        (begin
          (if (> amount u0)
            (try! (contract-call? .reward-token mint amount tx-sender))
            true
          )
          (map-set staked
            { token-id: token-id }
    ...
    (begin
      ...
      (let (
        ...
        (begin
          ...
          (try! (contract-call? .public-mint-nft transfer token-id (as-contract tx-sender) tx-sender))
