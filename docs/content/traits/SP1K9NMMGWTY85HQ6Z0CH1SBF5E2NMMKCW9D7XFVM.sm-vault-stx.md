...
;; TRAITS
;; ============================================================================
(impl-trait .sm-vault-traits.reserve)
(impl-trait .sm-vault-traits.tokenized-vault)
(impl-trait .sm-vault-traits.lending)
(impl-trait .sm-vault-traits.flashloan)
(use-trait flash-callback .sm-vault-traits.flash-callback)
...
;; -- Core configuration
(define-constant UNDERLYING .sm-wstx)
...
(define-private (receive-underlying (amount uint) (account principal))
  (begin
    (try! (contract-call? .sm-wstx transfer amount account current-contract none))
    (ok true)))

(define-private (send-underlying (amt uint) (account principal))
  (begin
    (try! (as-contract? ((with-stx amt))
      (try! (contract-call? .sm-wstx transfer amt tx-sender account none))
      true))
    (ok true)))

(define-private (ubalance)
  (unwrap-panic (contract-call? .sm-wstx get-balance current-contract)))
      ...
      (if (get accrue states)
          ...
          (let ((next (next-index))
            ...
            (if (not (is-eq lidx nliq))
                ...
                false)
            (if (> treasury-lp u0)
                (try! (ft-mint? zft treasury-lp .sm-dao-treasury))
                false)
...
(define-public (flashloan
  ...
  (let ((states (var-get pause-states))
    ...
    (if (> fee u0)
      (try! (send-underlying fee .sm-dao-treasury))
      false)
