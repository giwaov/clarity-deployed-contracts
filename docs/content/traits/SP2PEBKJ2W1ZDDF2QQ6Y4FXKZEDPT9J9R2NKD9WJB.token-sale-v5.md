---
title: "Trait token-sale-v5"
draft: true
---
```
;; Token Sale Contract V3 - SNTL Token Sale & Distribution
;; Multiple sale mechanisms: ICO, DEX listing, Stake-to-Earn
;; V3: Uses transfer instead of mint (tokens pre-loaded to contract)
...
(define-public (purchase-tokens (amount uint))
  (let
    ...
    (let
      ...
      (try! (as-contract (contract-call? sentinel-token transfer tokens-received-adjusted tx-sender buyer none)))
