---
title: "Trait cf-vault-setdev-v0"
draft: true
---
```
...
(use-trait wrapper-trait .cf-traits-v0.wrapper-trait)
...
(define-private (calculate-fee (amount uint))
    (let (
            (fee-bps (contract-call? .cf-helpers-state-v0 get-client-fee-bps CLIENT))
...
(define-private (assert-valid-caller)
    (let ((user (contract-call? .cf-helpers-state-v0 get-user-id-by-address tx-sender)))
        (asserts!
            (or
                (contract-call? .cf-helpers-state-v0 is-cofund-admin tx-sender)
...
(define-public (execute-transaction
    ...
    (begin
        ...
        (let (
                ...
                (policy (unwrap!
                    (contract-call? .cf-helpers-state-v0 get-policy CLIENT
                        policy-id
                    )
                    ERR_INVALID_POLICY
                ))
    ...
    (begin
        ...
        (let (
            ...
            (let (
                    ...
                    (fee-recipient (contract-call? .cf-helpers-state-v0
...
(define-private (verify-transaction-signature
    ...
    (begin
        ...
        (asserts!
            (is-none (contract-call? .cf-helpers-state-v0 get-used-auth-ids CLIENT auth-id))
            ERR_AUTH_ID_REPLAY
        )
        ...
        ;; update auth-id
        (unwrap!
            (contract-call? .cf-helpers-state-v0 set-auth-id CLIENT "vault"
                auth-id
            )
            ERR_AUTH_ID_REPLAY
        )
...
(define-private (verify-transaction-signature-helper
    ...
    (match signed-response
        ok-response (let (
                ...
                (user-id (get user-id
                    (unwrap!
                        (contract-call? .cf-helpers-state-v0
                            get-user-id-by-address signer-address
                        )
                        ERR_INVALID_USER
                    )))
                (user (unwrap!
                    (contract-call? .cf-helpers-state-v0 get-user CLIENT user-id)
                    ERR_INVALID_USER
                ))
            )
