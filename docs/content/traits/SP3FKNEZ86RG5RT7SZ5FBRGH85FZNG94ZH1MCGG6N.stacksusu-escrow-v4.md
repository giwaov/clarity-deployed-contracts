---
title: "Trait stacksusu-escrow-v4"
draft: true
---
```
...
(define-public (process-payout 
  ...
  (let
    (
      ...
      (treasury (contract-call? .stacksusu-admin-v4 get-treasury))
    )
        ...
        (begin
          (if (> admin-fee u0)
            (match (as-contract (stx-transfer? admin-fee tx-sender treasury))
              fee-success (begin (unwrap-panic (contract-call? .stacksusu-admin-v4 record-fee admin-fee)) true)
              fee-error true
            )
            true
          )
        ...
        (begin
          (if (> total-fees u0)
            (match (as-contract (stx-transfer? total-fees tx-sender treasury))
              fee-success (begin (unwrap-panic (contract-call? .stacksusu-admin-v4 record-fee total-fees)) true)
              fee-error true
            )
            true
          )
