---
title: "Trait dlmm-liquidity-router-v-0-1"
draft: true
---
```
;; dlmm-liquidity-router-v-0-1
...
(define-private (fold-add-liquidity-multi
  ...
  (let (
    ...
    (add-liquidity-result (try! (contract-call? .dlmm-core-v-0-1 add-liquidity (get pool-trait position) (get x-token-trait position) (get y-token-trait position) bin-id (get x-amount position) (get y-amount position) (get min-dlp position) (get max-x-liquidity-fee position) (get max-y-liquidity-fee position))))
...
(define-private (fold-add-relative-liquidity-multi
  ...
  (let (
    ...
    (bin-id-check (asserts! (and (>= bin-id MIN_BIN_ID) (<= bin-id MAX_BIN_ID)) ERR_INVALID_BIN_ID))
    (add-liquidity-result (try! (contract-call? .dlmm-core-v-0-1 add-liquidity pool-trait (get x-token-trait position) (get y-token-trait position) bin-id (get x-amount position) (get y-amount position) (get min-dlp position) (get max-x-liquidity-fee position) (get max-y-liquidity-fee position))))
    (updated-result (unwrap! (as-max-len? (append result-data add-liquidity-result) u350) ERR_RESULTS_LIST_OVERFLOW))
  )
    (ok updated-result)
  )
...
(define-private (fold-add-relative-liquidity-same-multi
  ...
  (let (
    ...
    (bin-id (+ active-bin-id (get active-bin-id-offset position)))
    (bin-id-check (asserts! (and (>= bin-id MIN_BIN_ID) (<= bin-id MAX_BIN_ID)) ERR_INVALID_BIN_ID))
    (add-liquidity-result (try! (contract-call? .dlmm-core-v-0-1 add-liquidity pool-trait x-token-trait y-token-trait bin-id (get x-amount position) (get y-amount position) (get min-dlp position) (get max-x-liquidity-fee position) (get max-y-liquidity-fee position))))
...
(define-private (fold-withdraw-liquidity-multi
  ...
  (let (
    ...
    (bin-id (get bin-id position))
    (bin-id-check (asserts! (and (>= bin-id MIN_BIN_ID) (<= bin-id MAX_BIN_ID)) ERR_INVALID_BIN_ID))
    (withdraw-liquidity-result (try! (contract-call? .dlmm-core-v-0-1 withdraw-liquidity (get pool-trait position) (get x-token-trait position) (get y-token-trait position) bin-id (get amount position) (get min-x-amount position) (get min-y-amount position))))
...
(define-private (fold-withdraw-relative-liquidity-multi
  ...
  (let (
    ...
    (bin-id (+ active-bin-id (get active-bin-id-offset position)))
    (bin-id-check (asserts! (and (>= bin-id MIN_BIN_ID) (<= bin-id MAX_BIN_ID)) ERR_INVALID_BIN_ID))
    (withdraw-liquidity-result (try! (contract-call? .dlmm-core-v-0-1 withdraw-liquidity pool-trait (get x-token-trait position) (get y-token-trait position) bin-id (get amount position) (get min-x-amount position) (get min-y-amount position))))
    (updated-result (unwrap! (as-max-len? (append result-data withdraw-liquidity-result) u350) ERR_RESULTS_LIST_OVERFLOW))
  )
    (ok updated-result)
  )
...
(define-private (fold-withdraw-liquidity-same-multi
  ...
  (let (
    ...
    (bin-id (get bin-id position))
    (bin-id-check (asserts! (and (>= bin-id MIN_BIN_ID) (<= bin-id MAX_BIN_ID)) ERR_INVALID_BIN_ID))
    (withdraw-liquidity-result (try! (contract-call? .dlmm-core-v-0-1 withdraw-liquidity (get pool-trait position) x-token-trait y-token-trait bin-id (get amount position) (get min-x-amount position) (get min-y-amount position))))
...
(define-private (fold-withdraw-relative-liquidity-same-multi
  ...
  (let (
    ...
    (bin-id (+ active-bin-id (get active-bin-id-offset position)))
    (bin-id-check (asserts! (and (>= bin-id MIN_BIN_ID) (<= bin-id MAX_BIN_ID)) ERR_INVALID_BIN_ID))
    (withdraw-liquidity-result (try! (contract-call? .dlmm-core-v-0-1 withdraw-liquidity pool-trait x-token-trait y-token-trait bin-id (get amount position) (get min-x-amount position) (get min-y-amount position))))
    (updated-results (unwrap! (as-max-len? (append (get results result-data) withdraw-liquidity-result) u350) ERR_RESULTS_LIST_OVERFLOW))
    (updated-x-amount (+ (get x-amount result-data) (get x-amount withdraw-liquidity-result)))
    (updated-y-amount (+ (get y-amount result-data) (get y-amount withdraw-liquidity-result)))
  )
...
(define-private (fold-move-liquidity-multi
  ...
  (let (
    ...
    (move-liquidity-result (try! (contract-call? .dlmm-core-v-0-1 move-liquidity (get pool-trait position) (get x-token-trait position) (get y-token-trait position) from-bin-id to-bin-id (get amount position) (get min-dlp position) (get max-x-liquidity-fee position) (get max-y-liquidity-fee position))))
...
(define-private (fold-move-relative-liquidity-multi
  ...
  (let (
    ...
    (to-bin-id-check (asserts! (and (>= to-bin-id MIN_BIN_ID) (<= to-bin-id MAX_BIN_ID)) ERR_INVALID_BIN_ID))
    (move-liquidity-result (try! (contract-call? .dlmm-core-v-0-1 move-liquidity pool-trait (get x-token-trait position) (get y-token-trait position) from-bin-id to-bin-id (get amount position) (get min-dlp position) (get max-x-liquidity-fee position) (get max-y-liquidity-fee position))))
    (updated-result (unwrap! (as-max-len? (append result-data move-liquidity-result) u350) ERR_RESULTS_LIST_OVERFLOW))
  )
    (ok updated-result)
  )
