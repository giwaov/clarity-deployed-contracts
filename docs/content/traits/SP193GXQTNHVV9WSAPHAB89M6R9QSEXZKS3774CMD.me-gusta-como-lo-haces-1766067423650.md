---
title: "Trait me-gusta-como-lo-haces-1766067423650"
draft: true
---
```
;; SUMAK AUDIO NFT CONTRACT
;; ME_GUSTA_COMO_LO_HACES Audio NFT

(define-non-fungible-token ME_GUSTA_COMO_LO_HACES uint)
...
(begin
  (unwrap! (nft-mint? ME_GUSTA_COMO_LO_HACES u1 DEPLOYER) (err ERR-NFT-MINT))
  (map-set cids u1 "QmYXP5z94HJqeSe37ypume9bcjt3nh9wsiZm8nR6fmfcLV")
...
(define-public (set-token-uri (token-id uint) (metadata-cid (string-ascii 256)))
  (begin
    ...
    (asserts! (is-some (nft-get-owner? ME_GUSTA_COMO_LO_HACES token-id)) (err ERR-NOT-FOUND))
...
(define-public (mint-additional (metadata-cid (string-ascii 256)))
  (let ((token-id (+ (var-get last-id) u1)))
    (asserts! (not (var-get locked)) (err ERR-CONTRACT-LOCKED))
    (try! (nft-mint? ME_GUSTA_COMO_LO_HACES token-id tx-sender))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ME_GUSTA_COMO_LO_HACES token-id)))
...
(define-public (buy-in-sat (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ME_GUSTA_COMO_LO_HACES id) (err ERR-NOT-FOUND)))
...
(define-private (is-owner (token-id uint) (user principal))
  (is-eq user (unwrap! (nft-get-owner? ME_GUSTA_COMO_LO_HACES token-id) false)))

(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ME_GUSTA_COMO_LO_HACES id sender recipient)
    success
      (begin
