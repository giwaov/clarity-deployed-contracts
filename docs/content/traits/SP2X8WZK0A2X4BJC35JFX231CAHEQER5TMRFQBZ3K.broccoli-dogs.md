...
(define-non-fungible-token broccoli-dogs uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmXaQzRvxPNNCzgLP5uBCgUVdoaqV19G3PwcrHUL7sKSCD/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? broccoli-dogs next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? broccoli-dogs token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? broccoli-dogs token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? broccoli-dogs token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? broccoli-dogs id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? broccoli-dogs id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? broccoli-dogs id) (err ERR-NOT-FOUND)))
