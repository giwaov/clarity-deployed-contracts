---
title: "Trait loTJ-stacks-wars-leo-vault"
draft: true
---
```
...
(define-constant POOL-SIZE u44000000000)
...
(define-public (join)
	(let
		...
		(if (is-eq sender DEPLOYER)
			(try! (contract-call? 'SP1AY6K3PQV5MRT6R4S671NWW2FRVPKM0BR162CT6.leo-token transfer POOL-SIZE sender (as-contract tx-sender) none))
			true
		)
...
(define-public (leave (signature (buff 65)))
	(let
		...
		(if (is-eq sender DEPLOYER)
			(try! (as-contract (contract-call? 'SP1AY6K3PQV5MRT6R4S671NWW2FRVPKM0BR162CT6.leo-token transfer POOL-SIZE tx-sender sender none)))
			true
		)
...
(define-public (claim (amount uint) (signature (buff 65)))
	(let
		...
		(try! (as-contract (contract-call? 'SP1AY6K3PQV5MRT6R4S671NWW2FRVPKM0BR162CT6.leo-token transfer fee-amount tx-sender FEE-WALLET none)))

		;; Transfer reward to player
		(try! (as-contract (contract-call? 'SP1AY6K3PQV5MRT6R4S671NWW2FRVPKM0BR162CT6.leo-token transfer reward-amount tx-sender sender none)))
