---
title: "Trait Yqk3-stacks-wars-welshcorgicoin-vault"
draft: true
---
```
...
(define-constant POOL-SIZE u117248000000)
...
(define-public (join)
	(let
		...
		(if (is-eq sender DEPLOYER)
			(try! (contract-call? 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token transfer POOL-SIZE sender (as-contract tx-sender) none))
			true
		)
...
(define-public (leave (signature (buff 65)))
	(let
		...
		(if (is-eq sender DEPLOYER)
			(try! (as-contract (contract-call? 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token transfer POOL-SIZE tx-sender sender none)))
			true
		)
...
(define-public (claim (amount uint) (signature (buff 65)))
	(let
		...
		(try! (as-contract (contract-call? 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token transfer fee-amount tx-sender FEE-WALLET none)))

		;; Transfer reward to player
		(try! (as-contract (contract-call? 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token transfer reward-amount tx-sender sender none)))
