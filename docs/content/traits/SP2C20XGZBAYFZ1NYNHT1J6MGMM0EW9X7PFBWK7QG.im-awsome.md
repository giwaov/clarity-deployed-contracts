---
title: "Trait im-awsome"
draft: true
---
```

;; title: Im awsome NFT Contract
;; version: 1.0
;; summary: A non-fungible token contract for Im awsome NFTs
...
;; token definitions
(define-non-fungible-token imawsome uint)
...
;; IPFS root for token metadata - uses {id} template pattern for token URI
;; Format: ipfs://CID/folder/ - wallets/marketplaces replace {id} with token number
(define-data-var ipfs-root (string-ascii 80) "ipfs://bafybeidajoaiyfkyl7mxsju57kqpbkxtheegzxsdvjvbxwbk562s5w5p7i/")
...
(define-data-var license-uri (string-ascii 80) "https://creativecommons.org/public-domain/cc0/")
(define-data-var license-name (string-ascii 40) "CC0")
...
(define-data-var split-count uint u2)
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? imawsome next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? imawsome token-id) false)))

(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? imawsome id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? imawsome id) false)))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? imawsome token-id tx-sender)))

;; Update the IPFS base URI
...
(define-public (buy-in-ustx (id uint))
  (let (
      (owner (unwrap! (nft-get-owner? imawsome id) (err ERR-NOT-FOUND)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? imawsome token-id)))

(define-read-only (get-last-token-id)
  (ok (- (var-get last-id) u1)))

;; Returns the token URI using {id} template pattern
;; Wallets and marketplaces replace {id} with the actual token number
;; Example: ipfs://QmXXX/folder/{id}.json -> ipfs://QmXXX/folder/1.json
;; This is the standard SIP-009 compliant approach used by major NFT collections
...
;; ============================================
;; INITIAL SPLIT PAYEES CONFIGURATION
;; These split payees were configured at deployment time
;; ============================================
(map-set split-payees u0 {payee: 'SP27ENRYMEGM9K6TVR7A8JEDTFXN5EA22KPJHVPE1, percentage: u200})
(map-set split-payees u1 {payee: 'SP3N3B77Y7Q2KDSQS5A630AN2HGZ96P36ZJAYK1MY, percentage: u300})
...
(define-public (admin-airdrop)
  (let
    ...
    (begin
      ...
      (try! (nft-mint? imawsome (+ last-nft-id u0) 'SP27ENRYMEGM9K6TVR7A8JEDTFXN5EA22KPJHVPE1))
      (map-set token-count 'SP27ENRYMEGM9K6TVR7A8JEDTFXN5EA22KPJHVPE1 (+ (get-balance 'SP27ENRYMEGM9K6TVR7A8JEDTFXN5EA22KPJHVPE1) u1))
      (try! (nft-mint? imawsome (+ last-nft-id u1) 'SP3N3B77Y7Q2KDSQS5A630AN2HGZ96P36ZJAYK1MY))
