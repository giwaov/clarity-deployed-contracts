---
title: "Trait keeper-5-1oyafqh3q-v-1-1"
draft: true
---
```
;; keeper-5-1oyafqh3q-v-1-1
...
(define-read-only (get-max-action-fee (action-contract principal))
  (ok (default-to (var-get default-max-action-fee) (map-get? max-action-fees action-contract)))
)
...
(define-public (execute-action-a
  ...
  (let (
    ;; Get owner, bitcoin, keeper, and action contract addresses
    ...
    (authorization-data (unwrap! (contract-call? 'SP3ESW1QCNQPVXJDGQWT7E45RDCH38QBK9HEJSX4X.keeper-5-helper-v-1-3 get-authorization-data action-contract) ERR_INVALID_HELPER_DATA))
    ...
    (action-check-b (asserts! (unwrap-panic (get-action-approved action-contract)) ERR_ACTION_NOT_APPROVED))

    ;; Assert action fee is less than or equal to max action fee
    (action-fee (get action-fee authorization-data))
    (max-action-fee (unwrap-panic (get-max-action-fee action-contract)))
