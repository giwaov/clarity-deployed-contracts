---
title: "Trait staging-dao-multisig-v0"
draft: true
---
```
;; ============================================================================
...
(use-trait proposal-script .staging-dao-traits-v0.proposal-script)
...
(define-private (check-dao-auth)
  (ok (asserts! (is-eq tx-sender .staging-dao-executor-v0) ERR-DAO)))
...
(define-public (add-signer (addr principal))
  (begin
    ...
    (print {
      ...
      data: {
        signer: addr,
        signers-count: (+ (var-get signer-count) u1),
...
(define-public (execute-impl-update)
  (let ((update (unwrap! (var-get pending-impl-update) ERR-SANITY-PROPOSAL)))
    ...
    (try! (contract-call? .staging-dao-executor-v0 set-impl (get new-impl update)))
...
(define-public (execute (id uint) (script <proposal-script>))
  (let ((proposal (unwrap-panic (map-get? proposals id)))
    ...
    (contract-call? .staging-dao-executor-v0 execute-proposal script)))
