---
title: "Trait stacks-index-oneclick-v7"
draft: true
---
```
;; =====================================================================
;; StacksIndex OneClick V7 - 5 Token Support (No Velar Router)
;; =====================================================================
;;
...
;; - WELSH: 4-hop via router-v-1-5 (dual stableswap + 1 xyk)
;; - DROID: 5-hop via router-v-1-2 swap-helper-c (stableswap + 3 xyk)
;;
;; NOTE: LEO and ROO removed due to velar router compatibility issues
;;
;; =====================================================================
...
(define-constant BITFLOW-ROUTER-V15 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.router-stableswap-xyk-multihop-v-1-5)
...
;; =====================
;; PRIVATE FUNCTIONS - Router V1.5 (Dual Stableswap)
;; =====================

;; Execute 4-hop swap via router v1.5 (swap-helper-a)
;; Route: stableswap-v14 -> stableswap-v12 -> xyk
;; Used for: WELSH
(define-private (execute-swap-4hop-v15
    (amount uint)
    (min-out uint)
    (reversed bool)
    (ss14-token-a <ft-trait>)
    (ss14-token-b <ft-trait>)
    (ss14-pool <stableswap-pool-trait-v14>)
    (ss12-token-a <ft-trait>)
    (ss12-token-b <ft-trait>)
    (ss12-pool <stableswap-pool-trait-v12>)
    (xyk-token-a <ft-trait>)
    (xyk-token-b <ft-trait>)
    (xyk-pool-a <xyk-pool-trait>)
  )
  (contract-call? BITFLOW-ROUTER-V15 swap-helper-a
    ...
    none
    reversed
    { a: ss14-token-a, b: ss14-token-b }
    { a: ss14-pool }
    { a: ss12-token-a, b: ss12-token-b }
    { a: ss12-pool }
    { a: xyk-token-a, b: xyk-token-b }
    { a: xyk-pool-a }
  )
...
;; - WELSH: 4-hop route via router v1.5 (dual stableswap)
...
(define-public (invest-diversified
    ...
    ;; === SWAP 4: WELSH (4-hop via v1.5) ===
    (welsh-amount uint)
    (welsh-min-out uint)
    (welsh-reversed bool)
    (welsh-ss14-token-a <ft-trait>)
    (welsh-ss14-token-b <ft-trait>)
    (welsh-ss14-pool <stableswap-pool-trait-v14>)
  ...
  (let
    ...
    (if (> welsh-amount u0)
      (let
        (
          (swap-result (try! (as-contract (execute-swap-4hop-v15
            welsh-amount welsh-min-out welsh-reversed
            welsh-ss14-token-a welsh-ss14-token-b welsh-ss14-pool
            welsh-ss12-token-a welsh-ss12-token-b welsh-ss12-pool
            welsh-xyk-token-a welsh-xyk-token-b welsh-xyk-pool-a))))
