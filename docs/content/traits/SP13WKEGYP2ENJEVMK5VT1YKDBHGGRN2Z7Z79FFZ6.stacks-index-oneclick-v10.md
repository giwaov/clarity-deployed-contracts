---
title: "Trait stacks-index-oneclick-v10"
draft: true
---
```
;; =====================================================================
;; StacksIndex OneClick V9 - 5 Token Support (Velar wSTX Route)
;; =====================================================================
;;
;; Supports 5 index tokens:
;; - wSTX: 2-hop via router-stableswap-velar-v-1-5 (stableswap + velar)
;; - sBTC: 3-hop via router-v-1-2 (swap-helper-b)
;; - ALEX: 3-hop via router-v-1-2 (swap-helper-b)
;; - WELSH: 3-hop via router-v-1-2 (swap-helper-b)
;; - DROID: 5-hop via router-v-1-2 (swap-helper-c)
;;
;; V9 FIX: wSTX now uses Velar route (like Bitflow webapp) for better liquidity
;; Route: USDCx -> aeUSDC (stableswap) -> wSTX (velar)
;;
;; =====================================================================
...
(define-constant BITFLOW-ROUTER-VELAR 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.router-stableswap-velar-v-1-5)
...
;; =====================
;; PRIVATE FUNCTIONS - Velar Router (for wSTX)
;; =====================

;; Execute 2-hop swap via Velar router (swap-helper-a)
;; Route: stableswap -> velar
;; Used for: wSTX (V9 FIX: better liquidity than XYK)
(define-private (execute-swap-2hop-velar
    (amount uint)
    (min-out uint)
    (reversed bool)
    (ss-token-a <ft-trait>)
    ...
    (velar-token-a <ft-trait>)
    (velar-token-b <ft-trait>)
    (velar-share-fee-to <share-fee-to-trait>)
  )
  (contract-call? BITFLOW-ROUTER-VELAR swap-helper-a
    amount
    min-out
    none
    reversed
    ...
    { a: ss-pool }
    { a: velar-token-a, b: velar-token-b }
    velar-share-fee-to
  )
)

;; =====================
;; PRIVATE FUNCTIONS - Router V1.2 (XYK)
;; =====================

;; Execute 3-hop swap via router v1.2 (swap-helper-b)
;; Route: stableswap -> xyk -> xyk
;; Used for: sBTC, ALEX, WELSH
...
;; - wSTX: 2-hop route via Velar router (V9 FIX!)
;; - sBTC: 3-hop route via router v1.2
;; - ALEX: 3-hop route via router v1.2
;; - WELSH: 3-hop route via router v1.2
;; - DROID: 5-hop route via router v1.2
;;
...
(define-public (invest-diversified
    ...
    ;; === SWAP 1: wSTX (2-hop via Velar) - V9 FIX! ===
    ;; Route: USDCx -> aeUSDC (stableswap) -> wSTX (velar)
    (wstx-amount uint)
    (wstx-min-out uint)
    (wstx-reversed bool)
    ...
    (wstx-velar-token-a <ft-trait>)
    (wstx-velar-token-b <ft-trait>)
    (wstx-velar-share-fee-to <share-fee-to-trait>)
  ...
  (let
    ...
    (if (> wstx-amount u0)
      (let
        (
          (swap-result (try! (as-contract (execute-swap-2hop-velar
            wstx-amount wstx-min-out wstx-reversed
            wstx-ss-token-a wstx-ss-token-b wstx-ss-pool
            wstx-velar-token-a wstx-velar-token-b wstx-velar-share-fee-to))))
