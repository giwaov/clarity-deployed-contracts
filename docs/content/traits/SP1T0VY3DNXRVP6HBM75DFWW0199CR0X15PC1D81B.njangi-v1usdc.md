...
(define-constant TOKEN_DECIMALS u1000000) ;; 10^6 - for converting token amounts to human-readable (Teiko and USDCx tokens have 6 decimals)
(define-constant INITIAL_VIRTUAL_USDCX u15000) ;; 15k USDCx (adjusted for bonding curve pricing - optimized for affordability)
...
(define-constant MIN_UNLOCK_THRESHOLD u1500) ;; Minimum threshold (1500 USDCx)
(define-constant MAX_UNLOCK_THRESHOLD u1000000) ;; Maximum cap (1M USDCx)

;; DATA VARIABLES
(define-data-var token-balance uint u0) ;; Trading pool (bonding curve)
(define-data-var usdcx-available-for-trading uint u0) ;; Real USDCx available for trading (derived from total - accumulated)
(define-data-var virtual-usdcx-amount uint INITIAL_VIRTUAL_USDCX) ;; Virtual balance for curve math
...
(define-data-var total-usdcx-invested-by-traders uint u0) ;; Lifetime total USDCx invested by all traders
(define-data-var total-usdcx-received-by-traders uint u0) ;; Lifetime total USDCx received by all traders from sells
(define-map curve-tokens-bought {holder: principal} uint) ;; Tracks tokens bought from bonding curve only
(define-map usdcx-invested {holder: principal} uint) ;; Tracks total USDCx spent on buys (lifetime, never resets)
(define-map current-usdcx-invested {holder: principal} uint) ;; Tracks USDCx spent for current position (resets to 0 on sell-all)
(define-map usdcx-received {holder: principal} uint) ;; Tracks total USDCx received from sells
...
(define-data-var total-usdcx-balance uint u0) ;; Explicitly track total USDCx in contract (for debugging balance mismatches)
...
;; max-cost: Maximum USDCx user is willing to pay (slippage protection)
(define-public (buy-tokens (token-amount-thousands uint) (max-cost uint))
  (begin
    ...
    (let (
      ...
      (current-usdcx-balance (+ (var-get usdcx-available-for-trading) (var-get virtual-usdcx-amount)))
      (current-token-balance (var-get token-balance))
      (k (* current-token-balance current-usdcx-balance))
      (new-token-balance (- current-token-balance tokens-out))
      (new-usdcx-balance (/ k new-token-balance))
      (net-usdcx-needed (- new-usdcx-balance current-usdcx-balance))
      (recipient tx-sender)
      (market-manager (unwrap! (var-get market-manager-address) ERR_NOT_INITIALIZED))
      (mm-fee (/ (* net-usdcx-needed u600) u10000)) ;; 6% fee to market maker
      (mh-fee (/ (* net-usdcx-needed u1500) u10000)) ;; 15% fee to majority holder pot
      (usdcx-after-fees (- net-usdcx-needed (+ mm-fee mh-fee))) ;; Net USDCx after 21% fees
    )
      ...
      (asserts! (> net-usdcx-needed u0) ERR_INVALID_AMOUNT)
      (asserts! (> usdcx-after-fees u0) ERR_INVALID_AMOUNT) ;; Ensure fees don't consume entire amount
      ;; Slippage protection: actual cost must not exceed max-cost
      (asserts! (<= net-usdcx-needed max-cost) ERR_SLIPPAGE_EXCEEDED)
      (try! (contract-call? 'SP120SBRBQJ00MCWS7TM5R8WJNTTKD5K0HFRC2CNE.usdcx 
        transfer net-usdcx-needed tx-sender DEX_ADDRESS (some 0x00)))
      ...
      ;; Update total USDCx balance tracking (contract receives: usdcx-after-fees + mh-fee, mm-fee paid out immediately)
      (var-set total-usdcx-balance (+ (var-get total-usdcx-balance) usdcx-after-fees mh-fee))
      ;; Derive usdcx-available-for-trading from total - accumulated (ensures it's always in sync)
      (var-set usdcx-available-for-trading (- (var-get total-usdcx-balance) (var-get accumulated-trading-fees)))
      ...
      ;; Track USDCx invested (total USDCx spent - lifetime, never resets)
      (let (
        (current-invested (default-to u0 (map-get? usdcx-invested {holder: tx-sender})))
      )
        (map-set usdcx-invested {holder: tx-sender} (+ current-invested net-usdcx-needed))
      )
      ;; Track current position usdcx invested (resets to 0 on sell-all)
      (let (
        (current-position-invested (default-to u0 (map-get? current-usdcx-invested {holder: tx-sender})))
      )
        (map-set current-usdcx-invested {holder: tx-sender} (+ current-position-invested net-usdcx-needed))
      )
      ;; Track total USDCx invested by all traders (global)
      (var-set total-usdcx-invested-by-traders (+ (var-get total-usdcx-invested-by-traders) net-usdcx-needed))
      (ok tokens-out)
    )
  )
)
...
;; Uses working bonding curve logic from testnet version
;; min-received: Minimum USDCx user is willing to receive after fees (slippage protection)
(define-public (sell-all-ownership (min-received uint))
  (begin
    ...
    ;; Cannot sell until there's actual USDCx liquidity from buys
    (asserts! (> (var-get usdcx-available-for-trading) u0) ERR_INSUFFICIENT_TRADING_LIQUIDITY)
    (let (
      ...
      (let (
        (available-usdcx (var-get usdcx-available-for-trading))
        (current-usdcx-balance (+ available-usdcx (var-get virtual-usdcx-amount)))
        (current-token-balance (var-get token-balance))
        (k (* current-token-balance current-usdcx-balance))
        (new-token-balance (+ current-token-balance user-curve-tokens))
        (new-usdcx-balance (/ k new-token-balance))
        (usdcx-out (- current-usdcx-balance new-usdcx-balance))
        (recipient tx-sender)
        (market-manager (unwrap! (var-get market-manager-address) ERR_NOT_INITIALIZED))
        ;; Cap usdcx-out to available real USDCx before calculating fees
        (capped-usdcx-out (if (>= usdcx-out available-usdcx) available-usdcx usdcx-out))
        (mm-fee (/ (* capped-usdcx-out u600) u10000)) ;; 6% fee to market maker
        (mh-fee (/ (* capped-usdcx-out u1500) u10000)) ;; 15% fee to majority holder pot
        (usdcx-after-fees (- capped-usdcx-out (+ mm-fee mh-fee))) ;; Net USDCx after 21% fees
        ;; Total amount we actually pay out (MM fee + user payment, MH fee stays in contract)
        (amount-to-pay-out (+ mm-fee usdcx-after-fees))
      )
        ;; Validate bonding curve calculation
        (asserts! (> new-token-balance u0) ERR_INSUFFICIENT_TRADING_LIQUIDITY)
        (asserts! (> usdcx-out u0) ERR_INVALID_AMOUNT)
        (asserts! (> usdcx-after-fees u0) ERR_INVALID_AMOUNT) ;; Ensure fees don't consume entire amount
        ...
        (asserts! (>= usdcx-after-fees min-received) ERR_SLIPPAGE_EXCEEDED)
        ;; Check liquidity: must have enough to pay MM fee + user payment
        (asserts! (>= available-usdcx amount-to-pay-out) ERR_INSUFFICIENT_TRADING_LIQUIDITY)
        (try! (contract-call? 'SP1T0VY3DNXRVP6HBM75DFWW0199CR0X15PC1D81B.teiko-token-stxcity
        ...
        (try! (as-contract (contract-call? 'SP120SBRBQJ00MCWS7TM5R8WJNTTKD5K0HFRC2CNE.usdcx 
          transfer usdcx-after-fees DEX_ADDRESS recipient (some 0x00))))
        ;; Add majority holder fee to pot (15% - stays in contract)
        (var-set accumulated-trading-fees (+ (var-get accumulated-trading-fees) mh-fee))
        ;; Update total USDCx balance tracking (contract pays out: amount-to-pay-out, keeps: mh-fee)
        (var-set total-usdcx-balance (- (var-get total-usdcx-balance) amount-to-pay-out))
        ;; Derive usdcx-available-for-trading from total - accumulated (ensures it's always in sync)
        (var-set usdcx-available-for-trading (- (var-get total-usdcx-balance) (var-get accumulated-trading-fees)))
        (var-set token-balance new-token-balance)
        ;; Track lifetime MM fees paid
        ...
        ;; Track USDCx received by user
        (let (
          (current-received (default-to u0 (map-get? usdcx-received {holder: tx-sender})))
        )
          (map-set usdcx-received {holder: tx-sender} (+ current-received usdcx-after-fees))
        )
        ;; Track total USDCx received by all traders (global)
        (var-set total-usdcx-received-by-traders (+ (var-get total-usdcx-received-by-traders) usdcx-after-fees))
        ;; Reset curve tokens tracking (user sold all their curve-bought tokens)
        (map-set curve-tokens-bought {holder: tx-sender} u0)
        ;; Reset current position usdcx invested (user sold out, position is now zero)
        (map-set current-usdcx-invested {holder: tx-sender} u0)
        (ok usdcx-after-fees)
      )
    )
  )
...
(define-public (claim-rewards)
  (begin
    ...
    (let (
      ...
      ;; Validate that contract has sufficient USDCx balance (rewards come only from accumulated-trading-fees)
      (let ((current-total-balance (var-get total-usdcx-balance)))
      ...
      (if is-mm-also-mh
        ...
        (let ((locked-tokens-to-refund (var-get locked-tokens)))
          ...
          ;; IMPORTANT: Rewards come ONLY from accumulated-trading-fees, not from usdcx-available-for-trading
          (var-set accumulated-trading-fees u0)
          ;; Update total USDCx balance tracking (contract pays out rewards from accumulated-trading-fees only)
          (var-set total-usdcx-balance (- (var-get total-usdcx-balance) fees-to-claim))
          ;; Derive usdcx-available-for-trading from total - accumulated (ensures it's always in sync)
          (var-set usdcx-available-for-trading (- (var-get total-usdcx-balance) (var-get accumulated-trading-fees)))
...
(define-read-only (get-sats-available-for-trading)
  (ok (var-get usdcx-available-for-trading))
)
...
;; Returns total USDCx balance tracked by contract (for debugging balance mismatches)
;; Should equal: usdcx-available-for-trading + accumulated-trading-fees
;; Can be compared against actual contract USDCx balance to identify discrepancies
(define-read-only (get-total-sbtc-balance)
  (ok (var-get total-usdcx-balance))
)
...
;; Returns usdcx needed to unlock (threshold system - 1M cap)
...
;; ========================================
;; Returns estimated USDCx needed to buy tokens (in thousands) at current price
...
;; Returns current position usdcx a user would receive if they sold all tokens now (after 21% fees: 6% MM + 15% MH, returns 0 if negative)
  ...
  (let (
    ...
    (if (is-eq user-curve-tokens u0)
      ...
      (let (
        ...
        (new-usdcx-balance (/ k new-token-balance))
        (usdcx-out (- current-usdcx-balance new-usdcx-balance))
        ;; Cap usdcx-out to available real USDCx before calculating fees (EXACT same as sell function)
        (capped-usdcx-out (if (>= usdcx-out available-usdcx) available-usdcx usdcx-out))
        (mm-fee (/ (* capped-usdcx-out u600) u10000)) ;; 6% fee to market maker
        (mh-fee (/ (* capped-usdcx-out u1500) u10000)) ;; 15% fee to majority holder pot
        (usdcx-after-fees (- capped-usdcx-out (+ mm-fee mh-fee))) ;; Net USDCx after 21% fees
        ...
        (ok (if (> usdcx-after-fees u0) usdcx-after-fees u0))
      )
    )
  )
)
...
;; ========================================
;; Returns lifetime total fees withdrawn by majority holders (in usdcx)
(define-read-only (get-lifetime-majority-holder-rewards-paid-out)
  (ok (var-get total-majority-holder-rewards-paid-out))
)

;; Returns lifetime total fees paid to market makers (in usdcx)
...
(define-read-only (get-total-realized-profit-by-traders)
  (let (
    (total-invested (var-get total-usdcx-invested-by-traders))
    (total-received (var-get total-usdcx-received-by-traders))
...
(define-read-only (get-total-rewards-received-by-all-users)
  (let (
    (traders-received (var-get total-usdcx-received-by-traders))
...
;; Returns lifetime total USDCx invested by all traders (in usdcx)
(define-read-only (get-total-sats-invested-by-all-users)
  (ok (var-get total-usdcx-invested-by-traders))
)

;; Returns lifetime realized profit for a specific user (usdcx-received - usdcx-invested, returns 0 if negative)
...
(define-read-only (get-user-lifetime-realized-profit (user principal))
  (let (
    (invested (default-to u0 (map-get? usdcx-invested {holder: user})))
    (received (default-to u0 (map-get? usdcx-received {holder: user})))
...
;; Uses current-usdcx-invested (resets to 0 on sell-all) for current position tracking
...
(define-read-only (get-user-current-unrealized-profit (user principal))
  (let (
    (invested (default-to u0 (map-get? current-usdcx-invested {holder: user})))
    (user-curve-tokens (default-to u0 (map-get? curve-tokens-bought {holder: user})))
  )
    (if (is-eq user-curve-tokens u0)
      ...
      (let (
        ...
        (new-usdcx-balance (/ k new-token-balance))
        (usdcx-out (- current-usdcx-balance new-usdcx-balance))
        (mm-fee (/ (* usdcx-out u600) u10000)) ;; 6% fee to market maker
        (mh-fee (/ (* usdcx-out u1500) u10000)) ;; 15% fee to majority holder pot
        (usdcx-after-fees (- usdcx-out (+ mm-fee mh-fee))) ;; Net USDCx after 21% fees
        (current-holdings-value usdcx-after-fees)
...
;; ========================================
;; Returns lifetime total USDCx invested by a specific user (in usdcx, includes fees) - lifetime, never resets
(define-read-only (get-user-lifetime-sbtc-invested (user principal))
  (ok (default-to u0 (map-get? usdcx-invested {holder: user})))
)

;; Returns current position USDCx invested by a specific user (in usdcx, includes fees) - resets to 0 on sell-all
;; Use this with get-user-current-tokens-bought to calculate average cost per token: current-usdcx-invested / tokens-held
(define-read-only (get-user-current-sats-invested (user principal))
  (ok (default-to u0 (map-get? current-usdcx-invested {holder: user})))
)

;; Returns lifetime total USDCx received by a specific user from sells (in usdcx)
(define-read-only (get-user-lifetime-sbtc-received (user principal))
  (ok (default-to u0 (map-get? usdcx-received {holder: user})))
)

;; Returns lifetime MH rewards received by a specific user (in usdcx)
...
;; Returns lifetime MM fees earned by a specific user (in usdcx)
