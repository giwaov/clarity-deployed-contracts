...
(define-constant TOKEN_NAME "TestDAO")
(define-constant TOKEN_SYMBOL "TDAO")
(define-constant TOKEN_DECIMALS u8)
(define-constant TOKEN_URI (some u"https://aibtc.dev/tokens/tdao.json"))
...
(define-fungible-token tdao MAX_SUPPLY)
...
(define-public (transfer
  ...
  (begin
    ...
    (try! (ft-transfer? tdao amount sender recipient))
...
(define-read-only (get-balance (account principal))
  (ok (ft-get-balance tdao account))
)

(define-read-only (get-total-supply)
  (ok (ft-get-supply tdao))
)
...
(define-public (distribute-founder (founder principal))
  (let (
    ...
    (try! (ft-mint? tdao amount founder))
...
(define-public (distribute-participant
  ...
  (let (
    ...
    (asserts! (not (var-get distribution-complete)) ERR_ALREADY_DISTRIBUTED)
    (asserts! (> amount u0) ERR_INVALID_AMOUNT)
    (try! (ft-mint? tdao amount participant))
    (var-set total-minted (+ (var-get total-minted) amount))
    (ok amount)
  )
)
...
(define-public (distribute-treasury (treasury principal))
  (let (
    ...
    (asserts! (is-eq tx-sender (var-get token-owner)) ERR_UNAUTHORIZED)
    (asserts! (not (var-get distribution-complete)) ERR_ALREADY_DISTRIBUTED)
    (try! (ft-mint? tdao amount treasury))
    (var-set total-minted (+ (var-get total-minted) amount))
    (ok amount)
  )
)
...
(define-public (distribute-verifier (verifier principal))
  (let (
    ...
    (asserts! (is-eq tx-sender (var-get token-owner)) ERR_UNAUTHORIZED)
    (asserts! (not (var-get distribution-complete)) ERR_ALREADY_DISTRIBUTED)
    (try! (ft-mint? tdao amount verifier))
    (var-set total-minted (+ (var-get total-minted) amount))
    (ok amount)
  )
)
...
(define-public (mint (amount uint) (recipient principal))
  (begin
    ...
    (asserts! (<= (+ (ft-get-supply tdao) amount) MAX_SUPPLY) ERR_INVALID_AMOUNT)
    (ft-mint? tdao amount recipient)
  )
)
