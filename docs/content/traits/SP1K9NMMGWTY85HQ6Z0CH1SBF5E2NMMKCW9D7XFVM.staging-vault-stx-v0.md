---
title: "Trait staging-vault-stx-v0"
draft: true
---
```
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;; -- Core configuration
;; Mainnet: References the staging wstx wrapper deployed in batch 6
(define-constant UNDERLYING .staging-wstx-v0)
...
(define-private (receive-underlying (amount uint) (account principal))
  (begin
    (try! (contract-call? .staging-wstx-v0 transfer amount account current-contract none))
    (ok true)))

(define-private (send-underlying (amt uint) (account principal))
  (begin
    (try! (as-contract? ((with-stx amt))
      (try! (contract-call? .staging-wstx-v0 transfer amt tx-sender account none))
      true))
    (ok true)))

(define-private (ubalance)
  (unwrap-panic (contract-call? .staging-wstx-v0 get-balance current-contract)))
