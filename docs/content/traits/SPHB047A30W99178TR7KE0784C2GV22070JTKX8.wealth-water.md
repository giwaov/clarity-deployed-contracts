...
(define-map sensor-operators principal bool)
...
(define-private (are-coordinates-valid (lat int) (lon int))
  (and 
    (>= lat MIN-LATITUDE)
    (<= lat MAX-LATITUDE)
    (>= lon MIN-LONGITUDE)
...
(define-private (are-parameters-valid
                 (ph uint) (oxygen uint) (turbidity uint) 
                 (temp uint) (conductivity uint) (tds uint))
  (and
        ...
        (if (is-eq (len current-admins) u1)
          ...
          (var-set admin-list 
                   (unwrap! (slice? current-admins u1 (len current-admins))
                           (err ERR-RECORD-NOT-FOUND))))
        (let ((before-target (unwrap! (slice? current-admins u0 admin-position) 
                                                    (err ERR-RECORD-NOT-FOUND)))
              (after-target (if (< (+ admin-position u1) (len current-admins))
                                             (unwrap! (slice? current-admins (+ admin-position u1) (len current-admins))
                                                     (err ERR-RECORD-NOT-FOUND))
                                             (list))))
          (var-set admin-list 
