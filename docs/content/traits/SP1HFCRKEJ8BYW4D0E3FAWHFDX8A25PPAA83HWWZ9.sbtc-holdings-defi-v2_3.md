---
title: "Trait sbtc-holdings-defi-v2_3"
draft: true
---
```
...
(define-read-only (get-user-sBTC-granite-v1 (user principal))
  (default-to u0
    (get amount
      (contract-call?
...
(define-read-only (get-user-sBTC-granite-usdcx (user principal))
  (default-to u0
    (get amount
      (contract-call?
...
(define-read-only (get-user-sBTC-granite (user principal))
  (+ (get-user-sBTC-granite-v1 user) (get-user-sBTC-granite-usdcx user))
)
...
(define-read-only (get-user-sBTC-zest-v1 (user principal))
  (unwrap!
    (contract-call?
...
(define-read-only (get-user-sBTC-zest-v2 (user principal))
  (get total
    (unwrap!
      (contract-call?
        'SP1A27KFY4XERQCCRCARCYD1CC5N7M6688BSYADJ7.v0-1-data
...
(define-read-only (get-user-sBTC-zest (user principal))
  (+ (get-user-sBTC-zest-v1 user) (get-user-sBTC-zest-v2 user))
)
