...
;; Helper to generate next unique digit
(define-private (get-next-unique-digit (seed uint) (offset uint) (existing-code (list 4 uint)))
    (let ((base-digit (mod (+ seed offset) u10)))
        ;; Try up to 10 different values to find a unique one
        (if (digit-exists-in-partial base-digit existing-code)
            (if (digit-exists-in-partial (mod (+ base-digit u1) u10) existing-code)
                (if (digit-exists-in-partial (mod (+ base-digit u2) u10) existing-code)
                    (if (digit-exists-in-partial (mod (+ base-digit u3) u10) existing-code)
                        (if (digit-exists-in-partial (mod (+ base-digit u4) u10) existing-code)
                            (if (digit-exists-in-partial (mod (+ base-digit u5) u10) existing-code)
                                (if (digit-exists-in-partial (mod (+ base-digit u6) u10) existing-code)
                                    (if (digit-exists-in-partial (mod (+ base-digit u7) u10) existing-code)
                                        (if (digit-exists-in-partial (mod (+ base-digit u8) u10) existing-code)
                                            (mod (+ base-digit u9) u10)
                                            (mod (+ base-digit u8) u10))
                                        (mod (+ base-digit u7) u10))
                                    (mod (+ base-digit u6) u10))
                                (mod (+ base-digit u5) u10))
                            (mod (+ base-digit u4) u10))
                        (mod (+ base-digit u3) u10))
                    (mod (+ base-digit u2) u10))
                (mod (+ base-digit u1) u10))
            base-digit)
    )
)

;; Helper to check if digit exists in partial code (0-4 elements)
(define-private (digit-exists-in-partial (digit uint) (code (list 4 uint)))
    (let ((len-code (len code)))
        (if (is-eq len-code u0)
            false
            (if (is-eq len-code u1)
                (is-eq (unwrap-panic (element-at? code u0)) digit)
                (if (is-eq len-code u2)
                    (or (is-eq (unwrap-panic (element-at? code u0)) digit)
                        (is-eq (unwrap-panic (element-at? code u1)) digit))
                    (if (is-eq len-code u3)
                        (or (is-eq (unwrap-panic (element-at? code u0)) digit)
                            (or (is-eq (unwrap-panic (element-at? code u1)) digit)
                                (is-eq (unwrap-panic (element-at? code u2)) digit)))
                        ;; len-code u4
                        (or (is-eq (unwrap-panic (element-at? code u0)) digit)
                            (or (is-eq (unwrap-panic (element-at? code u1)) digit)
                                (or (is-eq (unwrap-panic (element-at? code u2)) digit)
                                    (is-eq (unwrap-panic (element-at? code u3)) digit))))
                    )
                )
            )
...
;; Generate unique code: 5 different digits from 0-9 (GUARANTEED unique)
(define-private (generate-unique-code (seed uint))
    (let ((base-seed (+ (+ (* seed u997) stacks-block-height) (var-get player-count))))
        ...
        (let ((d0 (mod base-seed u10)))
            ;; Generate second digit (different from d0)
            (let ((d1 (get-next-unique-digit base-seed u17 (list d0))))
                ;; Generate third digit (different from d0, d1)
                (let ((d2 (get-next-unique-digit base-seed u41 (list d0 d1))))
                    ;; Generate fourth digit (different from d0, d1, d2)
                    (let ((d3 (get-next-unique-digit base-seed u89 (list d0 d1 d2))))
                        ;; Generate fifth digit (different from d0, d1, d2, d3)
                        (let ((d4 (get-next-unique-digit base-seed u199 (list d0 d1 d2 d3))))
                            (list d0 d1 d2 d3 d4)
                        )
                    )
                )
