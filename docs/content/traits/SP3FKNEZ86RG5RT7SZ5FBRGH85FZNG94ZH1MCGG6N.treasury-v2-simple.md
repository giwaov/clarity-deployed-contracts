---
title: "Trait treasury-v2-simple"
draft: true
---
```
;; Treasury V2 Contract
;; Fund management with creator earnings (0.01 STX)
;; Upgraded with version tracking and daily limits
;; Based on working treasury contract
...
(define-constant ERR-DAILY-LIMIT-EXCEEDED (err u307))
(define-constant ERR-NOT-SIGNER (err u308))
...
;; Daily withdrawal limit: 50,000 STX
(define-constant DAILY-LIMIT u50000000000)
...
(define-public (deposit-no-fee (amount uint))
  (let (
    ...
    (print { event: "deposit-no-fee", version: CONTRACT-VERSION, depositor: depositor, amount: amount })
    (ok amount)
  )
)
...
(define-public (emergency-withdraw (amount uint) (recipient principal))
  (begin
    ...
    (print { event: "emergency-withdrawal", version: CONTRACT-VERSION, recipient: recipient, amount: amount })
    (ok amount)
  )
)
