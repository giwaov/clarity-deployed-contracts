...
(define-read-only (get-price (symbol (string-ascii 10)))
	(let ((feed (unwrap! (map-get? price-feeds symbol) (err u404))))
		(asserts! (< (- stacks-block-height (get timestamp feed)) (var-get price-staleness-threshold)) err-stale-price)
		(ok (get price feed))))
...
(define-read-only (is-price-fresh (symbol (string-ascii 10)))
	(match (map-get? price-feeds symbol)
		feed (< (- stacks-block-height (get timestamp feed)) (var-get price-staleness-threshold))
		false))
