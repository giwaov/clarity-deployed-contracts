---
title: "Contract keeper-5-k3l1cze8a-v-1-1"
draft: true
---
Deployer: SP16BPKS1DN5AYQ5MDHFEYXTSP352QG6JS2E0N8YP


 



Block height: 5023250 (2025-11-28T20:29:44.000Z)

Source code: {{<contractref "keeper-5-k3l1cze8a-v-1-1" SP16BPKS1DN5AYQ5MDHFEYXTSP352QG6JS2E0N8YP keeper-5-k3l1cze8a-v-1-1>}}

Functions:

* is-keeper-or-owner _private_
* execute-action-a _public_
* set-action-approved _public_
* set-all-actions-approved _public_
* set-bitcoin-address _public_
* set-default-max-action-fee _public_
* set-keeper-address _public_
* set-keeper-authorized _public_
* set-max-action-fee _public_
* set-owner-address _public_
* withdraw-tokens _public_
* get-action-approved _read_only_
* get-all-actions-approved _read_only_
* get-bitcoin-address _read_only_
* get-default-max-action-fee _read_only_
* get-keeper-address _read_only_
* get-keeper-authorized _read_only_
* get-max-action-fee _read_only_
* get-owner-address _read_only_
