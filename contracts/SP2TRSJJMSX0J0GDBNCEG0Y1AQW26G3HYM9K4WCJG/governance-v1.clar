...
(define-private (execute-update-pyth-token-price-feed (proposal-id (buff 32)))
  (let ((pyth-data (unwrap-panic (map-get? update-pyth-feed proposal-id))))
    (contract-call? 'SP26NGV9AFZBX7XBDBS2C7EC7FCPSAV9PKREQNMVS.pyth-adapter-v1 update-price-feed-id (get token pyth-data) (get feed pyth-data) (get max-confidence-ratio pyth-data))
))
...
(define-private (execute-update-time-delta (proposal-id (buff 32)))
  (let ((time-delta (unwrap-panic (map-get? update-time-delta-params proposal-id))))
    (contract-call? 'SP26NGV9AFZBX7XBDBS2C7EC7FCPSAV9PKREQNMVS.pyth-adapter-v1 update-time-delta time-delta)
))

(define-private (execute-set-lp-cap (proposal-id (buff 32)))
  (let ((data (unwrap-panic (map-get? cap-data proposal-id))))
    (contract-call? 'SP26NGV9AFZBX7XBDBS2C7EC7FCPSAV9PKREQNMVS.withdrawal-caps-v1 set-lp-cap (get factor data))
))

(define-private (execute-set-debt-cap (proposal-id (buff 32)))
  (let ((data (unwrap-panic (map-get? cap-data proposal-id))))
    (contract-call? 'SP26NGV9AFZBX7XBDBS2C7EC7FCPSAV9PKREQNMVS.withdrawal-caps-v1 set-debt-cap (get factor data))
))
...
(define-private (execute-set-collateral-cap (proposal-id (buff 32)))
  (let (
    ...
    (contract-call? 'SP26NGV9AFZBX7XBDBS2C7EC7FCPSAV9PKREQNMVS.withdrawal-caps-v1 set-collateral-cap collateral factor)
))

(define-private (execute-set-refill-time-window (proposal-id (buff 32)))
  (let ((data (unwrap-panic (map-get? cap-data proposal-id))))
    (contract-call? 'SP26NGV9AFZBX7XBDBS2C7EC7FCPSAV9PKREQNMVS.withdrawal-caps-v1 set-refill-time-window (get factor data))
))

(define-private (execute-set-decay-time-window (proposal-id (buff 32)))
  (let ((data (unwrap-panic (map-get? cap-data proposal-id))))
    (contract-call? 'SP26NGV9AFZBX7XBDBS2C7EC7FCPSAV9PKREQNMVS.withdrawal-caps-v1 set-decay-time-window (get factor data))
))
...
(define-private (execute-update-flash-loan-fee (proposal-id (buff 32)))
  (let ((data (unwrap-panic (map-get? flash-loan-fee-update proposal-id))))
    (contract-call? 'SP26NGV9AFZBX7XBDBS2C7EC7FCPSAV9PKREQNMVS.flash-loan-v1 update-fee data)
))
...
(define-private (execute-add-flash-loan-contract (proposal-id (buff 32)))
  (let ((data (unwrap-panic (map-get? flash-loan-contract-update proposal-id))))
    (contract-call? 'SP26NGV9AFZBX7XBDBS2C7EC7FCPSAV9PKREQNMVS.flash-loan-v1 set-allowed-contract data)
))

(define-private (execute-remove-flash-loan-contract (proposal-id (buff 32)))
  (let ((data (unwrap-panic (map-get? flash-loan-contract-update proposal-id))))
    (contract-call? 'SP26NGV9AFZBX7XBDBS2C7EC7FCPSAV9PKREQNMVS.flash-loan-v1 remove-allowed-contract data)
))
...
(define-private (execute-allow-any-contract-flash-loan (proposal-id (buff 32)))
  (let ((data (unwrap-panic (map-get? flash-allow-disable-contract-update proposal-id))))
    (contract-call? 'SP26NGV9AFZBX7XBDBS2C7EC7FCPSAV9PKREQNMVS.flash-loan-v1 update-allow-any-contract data)
))
...
(define-private (approve-threshold-met (proposal-id (buff 32)))
  (let (
      ...
      (total-count (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.meta-governance-v1 governance-multisig-count))
...
(define-public (initiate-proposal-to-update-pyth-time-delta (expires-in uint) (time-delta uint))
  (let (
    ...
    (asserts! (>= time-delta (contract-call? 'SP26NGV9AFZBX7XBDBS2C7EC7FCPSAV9PKREQNMVS.pyth-adapter-v1 get-pyth-minimum-time-delta)) ERR-MINIMUM-PYTH-PRICE-DELTA)
    (try! (create-proposal proposal-id action expires-in))
