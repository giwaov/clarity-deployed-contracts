...
;; Define the NFT with the new name
(define-non-fungible-token RamuBTC uint)
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? RamuBTC token-id))
)
...
(define-public (transfer
  ...
  (begin
    ...
    (nft-transfer? RamuBTC token-id sender recipient)
  )
)
...
(define-public (mint (recipient principal))
  ...
  (let ((token-id (+ (var-get last-token-id) u1)))
    ...
    (try! (nft-mint? RamuBTC token-id recipient))
