...
;; title: genzzz NFT Contract
;; version: 1.0
;; summary: A non-fungible token contract for genzzz NFTs
...
;; token definitions
(define-non-fungible-token genzzz uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/bafybeif7ymkroo5hjtp7kqr3ubsfo5tlelt2dicqoj3dbz3jteu7yafo3e/json/")
...
(define-data-var license-name (string-ascii 40) "CC-BY-NC-NONCOMMERCIAL")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? genzzz next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? genzzz token-id) false)))

(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? genzzz id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? genzzz id) false)))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? genzzz token-id tx-sender)))
...
(define-public (buy-in-ustx (id uint))
  (let (
      (owner (unwrap! (nft-get-owner? genzzz id) (err ERR-NOT-FOUND)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? genzzz token-id)))
