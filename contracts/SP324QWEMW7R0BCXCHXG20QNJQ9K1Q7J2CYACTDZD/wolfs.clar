...
(define-non-fungible-token wolfs uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmZShjDUUERPWxxxUQ4WWALW8aY83jD5kbkQcQ2nr4692g/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? wolfs next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? wolfs token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? wolfs token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? wolfs token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? wolfs id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? wolfs id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? wolfs id) (err ERR-NOT-FOUND)))
